...
UniteAdminRev = new function(){
	...
	********************************/
	 ...
	 * TODO: Fix upload popover glitch
	 * TODO: Reskin uploader popover
	 */

	var openMageImageDialog = function(title, onInsert, isMultiple, fileType) {

        var uploadFileType = typeof fileType == 'undefined' ? 'image' : 'video';
        var uploadFileUrl = revMageImageUploadUrl.replace('[type]', uploadFileType);
		...
		Mediabrowser.prototype.insert = function(event) {
			...
			if (event != undefined) {
				...
				$$('div.selected.filecnt').each(function (e) {
					div = $(e.id);
				});
			}
			if ($(div.id) == undefined) {
		...
		jQuery('*').each(function() {
			var zIndex = parseInt(jQuery(this).css('zIndex'), 10);
			if(zIndex > zIndexMax)
			{
				zIndexMax = zIndex;
			}
		...
		MediabrowserUtility.openDialog(uploadFileUrl, false, false, title, {zIndex: zIndexMax});
	}
    ...
    t.duplicateObject = function(obj) {
        var duplicate = t.removePrototypeMethods(jQuery.extend({}, obj));
        return duplicate;
    };
