...
class WebVision_CustomerRegistrationNotification_Model_Observer
{
    /**
     * @var string
     */
    ...
    public function __construct()
    {
        ...
        $this->_customerEmail = Mage::getStoreConfig('customer/registration_notification/customer_email');
    ...
    public function customerRegisterSuccess(Varien_Event_Observer $observer)
    {
        ...
        $this->sendCustomerRegistrationNotification(
            $this->getCustomerData($customer->getEntityId()),
    ...
    protected function sendCustomerRegistrationNotification($customerData, $storeId)
    {
        ...
        if ($this->_customerEmail) {
            $mailer->setSender(array(
                'name'  => $customerData['customer']->getFirstname() . ' ' . $customerData['customer']->getLastname(),
                'email' => $customerData['customer']->getEmail(),
            ));
        } else {
	...
	$customerArr['cust_group'] = $group->getCode();
	Mage::log($customer->getGroupId(), null, 'mylogfile.log');

        if($customer->getGroupId() == "3"){
		$customerArr['profile_url'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'custom/index/artist/id/'.$customer_id.'/';
	}elseif($customer->getGroupId() == "2" ){
		$customerArr['profile_url'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'custom/index/details/id/'.$customer_id.'/';
	}
