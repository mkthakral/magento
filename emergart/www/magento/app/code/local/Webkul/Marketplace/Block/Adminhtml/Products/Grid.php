...
class Webkul_Marketplace_Block_Adminhtml_Products_Grid extends Mage_Adminhtml_Block_Widget_Grid {
        ...
        foreach ($this->getCollection() as $item) {
            ...
            if(!(is_null($item->getmageproductid())) && $item->getmageproductid() != 0){
                 if($item->getstatus() == 1){
                    ...
                    $item->status = sprintf('<a href="%s" title="'.$helper->__('Click to Approve').'" onclick="return confirm("'.$helper->__('You sure?').'")">'.$helper->__('Unapproved').'</a>',$this->getUrl('adminhtml/marketplace_products/approve/id/' . $item->getmageproductid()));
                }
    ...
    protected function _prepareMassaction()  {
        ...
        $this->getMassactionBlock()->addItem('delete', array(
           ...
           'url'      => $this->getUrl('adminhtml/marketplace_products/massapprove')
        ));
        return $this;
    }
