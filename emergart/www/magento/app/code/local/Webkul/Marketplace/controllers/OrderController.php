			...
			try{
				...
				if(count($seller_orders)){
					...
					if($this->getRequest()->getParam('api_shipment')){
						Mage::dispatchEvent('generate_api_shipment',array(
													'api_shipment'=>$this->getRequest()->getParam('api_shipment'),
													'orderid'=>$orderid,
													));
					...
					if($this->getRequest()->getParam('api_shipment') && $trackingid == ''){
						...
						if($order->canUnhold()) { 
							...
							if(count($itemsarray)>0){													
								...
								$courrier="custom";
								$seller_collection=Mage::getModel('marketplace/order')->getCollection()
									 ->addFieldToFilter('seller_id',array('eq'=>$customerid))
									 ->addFieldToFilter('order_id',array('eq'=>$orderid));
