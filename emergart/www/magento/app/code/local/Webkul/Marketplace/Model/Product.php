...
class Webkul_Marketplace_Model_Product extends Mage_Core_Model_Abstract
{
	...
	//	if(isset($wholedata['status']) && $wholedata['status'] && $id){
		if(isset($wholedata['status']) && $wholedata['status']){
			$status=$wholedata['status']; 
			 Mage::log('----IF----'.$status, null, 'mylogfile.log');
		}
		else{		
			$status=Mage::helper('marketplace')->getIsProductApproval()? 2:1;
                        Mage::log('----ELSE----'.$status, null, 'mylogfile.log');
		}
	        Mage::log('Product Status:'.$status, null, 'mylogfile.log');	
		...
		if(isset($_FILES['images']['name']) && $_FILES['images']['name']){
			...
			foreach($images_arr['name'] as $image){
				if($images_arr['tmp_name'][$i] != ''){
					...
					$target1 = $target.$image_name.".".strtolower($splitname[1]);
     ...
     */
	private function _addImages($product_id,$defaultimage,$storeId)
	{
	...
	private function AddImages($id,$storeId,$rates,$currentCurrencyCode,$wholedata)		
	{
		...
		$product = Mage::getModel('catalog/product')->load($id); //Load product.	

		$product_website_ids = 	$product->getWebsiteIds();

		$product_store_website_ids = $product->getStore()->getWebsiteId();

		$product_store_id = 	$product->getStoreId();
		...
		foreach($wholedata['samples'] as $key=>$samples)	{
			...
			$sampleModel->setWebsiteId($product_store_website_ids);
			$sampleModel->setProductWebsiteIds($product_website_ids);

			$sampleModel->setTitle($samples['title']);
			if($samples['type']=='file')	{
				if (file_exists($samples['file'])) {
					$mainsample_arr = explode('mainsample/',$samples['file']);
					$newfilename=$mainsample_arr[1];
			...
			$sampleModel->setStoreId($product_store_id);		
			$sampleModel->save();
		}
		foreach($wholedata['link'] as $key=>$links)	{
			...
			$web_id=Mage::app()->getStore(Mage_Catalog_Model_Abstract::DEFAULT_STORE_ID)->getWebsiteId().",".$storeId;
			$linkModel->setWebsiteId($product_store_website_ids);
			$linkModel->setProductWebsiteIds($product_website_ids);
			...
			if(isset($links['type']) && $links['type']=='file')	{
				if (file_exists($links['file'])) {
					$links_arr = explode('links/',$links['file']);
					$newfilename=$links_arr[1];
			...
			if(isset($links['sample']['type']) && $links['sample']['type']=='file'){
				if (file_exists($links['sample']['file'])) {
					$links_arr = explode('sample/',$links['sample']['file']);
					$newfilename=$links_arr[1];
			...
			$linkModel->setStoreId($product_store_id);
			$linkModel->save();
		}
	}
	...
	public function approveSimpleProduct($id)
	{
		...
		$emailTemp = Mage::helper('marketplace')->getWhenProductApprovedTemplate();
		$emailTempVariables = array();
