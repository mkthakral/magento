...
Class Webkul_Marketplace_Model_Observer
{
	public function CustomerRegister($observer)
	{
		$data=Mage::getSingleton('core/app')->getRequest();
			...
			if($data->getParam('wantpartner')==1 or $data->getParam('wantpartner')==2){//add custome code
			$customer = $observer->getCustomer();
			if($data->getParam('wantpartner')==1)
			{
				$customer->setGroupId(3);//add custome code
				$customer->save();
	...
	public function afterMultiShippingPlaceOrder($observer) 
	{
		$quoteId = Mage::getSingleton('checkout/session')->getLastQuoteId();
		$quote = Mage::getModel('sales/quote')->load($quoteId);
		if ($quote->getIsMultiShipping() == 1 || Mage::getSingleton('checkout/session')->getQuote()->getIsMultiShipping() == 1) { 
			$helper = Mage::helper('marketplace');
