...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
	...
	// additional reCAPTCHA validation, tis should actually be in it's own function, but I've added it here for simplicity

        // Magento uses this method for a few different requests, so make sure it's limited only to the 'createpost' action
        $action = Mage::app()->getRequest()->getActionName();
        if ( $action == 'createpost' ) { // restrict to the registration page only
			
			
			if(!substr_compare($this->getEmail(), "qq.com", -strlen( "qq.com" ))){
				$errors[] = Mage::helper('customer')->__('This email address is considred spam. Registration rejected.');
			}
			
            $captcha = Mage::app()->getRequest()->getPost('g-recaptcha-response', 1);
            if ( $captcha == '' ) {
                // if the field is empty, add an error which will be displayed at the top of the page
                $errors[] = Mage::helper('customer')->__('Please check the CAPTCHA checkbox to continue');
            } else {
                $secret = '6LdE_FwUAAAAAJDN_K2N69RlMqKckvC5VY0G4Grj';
                ...
                if ( trim( $result['success'] ) != true ) {
                    ...
                    $errors[] = Mage::helper('customer')->__('CAPTCHA verification failed.');
                }
            }
        }
