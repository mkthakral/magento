...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    protected function _welcomeCustomer(Mage_Customer_Model_Customer $customer, $isJustConfirmed = false)
    {
        ...
        $successUrl = $this->_getUrl('customer/account/dashbord');
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                $this->_getSession()->setCustomer($customer)
                    ->addSuccess($this->__('Successfully Saved'));
	...
	private function saveArtistProfileURL(){
		
		$customerGroudId = $this->_getSession()->getCustomer()->getGroupId();
		//Customer is Artist
		if($customerGroudId == "3"){
			$artistCustomerId = $this->_getSession()->getCustomer()->getId();
			$artistRewriteUniquePattern = "artist-profile-id-$artistCustomerId";
			$newProfilePageURL = 'artist/'.$this->getRequest()->getPost('profile-page-url');
			$rewrite = Mage::getModel('core/url_rewrite')->loadByIdPath($artistRewriteUniquePattern);
			//Mage::log("newProfilePageURL".$newProfilePageURL,null,'URL-Rewrite.log');
			//re-write exists
			if ($rewrite->getId()){
	...
	{	 
		 $this->loadLayout();
		 $this->getLayout()->getBlock('head')->setTitle("Help");
		 $this->renderLayout();
...
public function sendrequestAction(){
	...
	$name=$data['name'];
	 ...
	 if($data['customer_group_id'] == "3"){
                $profile_url = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'custom/index/artist/id/'.$data['customer_id'].'/';
        }elseif($data['customer_group_id'] == "2" ){
                $profile_url = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'custom/index/details/id/'.$data['customer_id'].'/';
        }

//  $to_email = Mage::getStoreConfig('trans_email/ident_general/email'); //fetch sender email Admin
	//Temp - Email Delay Fix- For Artist
//	$to_email = 'artvalero2@gmail.com';
        $from_email = Mage::getStoreConfig('trans_email/ident_general/email');
 $to_email = Mage::getStoreConfig('trans_email/ident_custom1/email');
// $to_email = 'info@emergart.net';
	
    
    $emailTemplate = Mage::getModel('core/email_template')->loadByCode('Request for approval | By Artist & Gallery | To Admin');
    $emailTemplateVariables = array();
    $emailTemplateVariables['cust_type'] = $usertype;
    $emailTemplateVariables['cust_name'] = $name;
    $emailTemplateVariables['cust_email'] = $email;
    $emailTemplateVariables['profile_url'] = $profile_url;
    ...
    $emailTemplate->send($to_email,'Emergart', $emailTemplateVariables);

  /*  Mage::log('$usertype: '.$usertype, null, 'emailDebug.log');
    Mage::log('$name: '.$name, null, 'emailDebug.log');
    Mage::log('$email: '.$email, null, 'emailDebug.log');
    Mage::log('$profile_url: '.$profile_url, null, 'emailDebug.log');
    Mage::log('$to_email: '.$to_email, null, 'emailDebug.log');*/
    
    $customer = Mage::getSingleton('customer/session')->getCustomer();
    $customer->setIssendrequest(1);
    $customer->save();

    Mage::getSingleton('core/session')->addSuccess('Request Submitted');
    $this->_redirectReferer();
}
 ...
 if (count($result1) == 0)
		   ...
		   $sqlPaymentSystem="INSERT INTO  ".$tableName." SET title='".$ques."',galeryid='".$galid."',answer='".addslashes($answer)."',question='".$question."'";
		     try {
		   ...
		   else
		   {
			 //  echo "already follow by you";
			  ...
			  $sqlPaymentSystem1="UPDATE ".$tableName." SET answer='".addslashes($answer)."',question='".$question."'  WHERE title=$ques and galeryid='".$galid."'";
            try {
                    $chkSystem = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlPaymentSystem1);
            } catch (Exception $e){
            echo $e->getMessage();
	...
	
	{
		...
		foreach($question as $ques){
		    ...
		    if (count($result1) == 0)
	       	    {
			$sqlPaymentSystem="INSERT INTO  ".$tableName." SET title='".$ques."',artistid='".$galid."',answer=\"".addslashes($answer)."\",question='".$question."'";
		        try {
			   $chkSystem = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlPaymentSystem);
		        } catch (Exception $e){
			  echo $e->getMessage();
			  ...
			  $sqlPaymentSystem1="UPDATE   ".$tableName." SET answer='".addslashes($answer)."',question='".$question."'  WHERE title=$ques"." and artistid='$galid'";
		          try {
                	    $chkSystem = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlPaymentSystem1);
		          } catch (Exception $e){
			            echo $e->getMessage();
