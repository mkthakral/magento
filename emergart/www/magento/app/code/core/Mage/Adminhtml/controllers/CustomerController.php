    ...
    public function saveAction()
    {
		...
		$isapproved=$customer->getIsapproved();
	        $customerGroupId = $customer->getGroupId();
                $accountApprovalEmailSentToCustomer = $customer->getIssentmail();
		//Send email to user if account is approved, if artist or gallery, if account approval email already not sent
		if($isapproved==1 and ($customerGroupId == 2 or $customerGroupId == 3) and $accountApprovalEmailSentToCustomer!=1){
 		    //Updating customer that account approval email is sent
		    $customer->setIssentmail(1);
	            $customer->save();
		    $emailTemplateVariables = array();
	 	    
		    if($customerGroupId == 3){
			 $emailTemplate  = Mage::getModel('core/email_template')->loadByCode('Account approved - Artist (mod by Art Valero)');
			 $emailTemplateVariables['cust_name'] = $customer->getName();
		    } elseif($customerGroupId == 2){
			 $emailTemplate  = Mage::getModel('core/email_template')->loadByCode('Account approved - Gallery (mod by Art Valero)');
			 $emailTemplateVariables['cust_name'] = $customer->getName();
                    }
		    ...
		    $emailTemplate->send($toemail,$customer->getName(), $emailTemplateVariables);
		}
