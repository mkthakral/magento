...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
	...
	//Send email to galleru who had endorsed this artwork
	$orderid= $session->getLastOrderId();
	$getOrder=Mage::getModel('sales/order')->load($orderid);
	$Incrementid = $getOrder->getIncrementId();
	...
	foreach($orderItems as $sItem) {
		...
		$productName= $nProduct->getName();
		$galcode=$nProduct->getGalcode();
		$productPrice="$".number_format($nProduct->getPrice(),2);
		$galleryData = Mage::getModel('customer/customer')->load($galcode);
		$galleryName = $galleryData->getName();
		$galleryEmail = $galleryData->getEmail();
		$marketplaceTable= Mage::getSingleton('core/resource')->getTableName('marketplace_product');
		$sqlQuery="SELECT * FROM ".$marketplaceTable." WHERE mageproductid='".$sItem->getProductId()."'";
		try {
			$connection = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlQuery);
			$tableResult = $connection->fetchall();
		}catch (Exception $e){
			echo $e->getMessage();
		}
		foreach($tableResult as $tableRow){
			$artistId=$tableRow['userid'];
		}
		$artistData=Mage::getModel('customer/customer')->load($artistId);
		$artistName=$artistData->getName();
		if($galcode!=""){
			$emailTemplate = Mage::getModel('core/email_template')->loadByCode('Artwork Sold Notification | To gallery');
			$emailTemplateVariables = array();
			$emailTemplateVariables['galleryName']= $galleryName;
			$emailTemplateVariables['artworkName'] = $productName;
			$emailTemplateVariables['artistName'] = $artistName;
			$emailTemplateVariables['productPrice'] = $productPrice;
			$processedTemplate = $emailTemplate->getProcessedTemplate($emailTemplateVariables);
			$emailTemplate->setSenderName('Emergart');
			$emailTemplate->setSenderEmail(Mage::getStoreConfig('trans_email/ident_general/email'));
			$emailTemplate->setType('html');
			$emailTemplate->setBody($processedTemplate);
			$emailTemplate->send($galleryEmail, $galleryName , $emailTemplateVariables);
		}
  	}
            ...
            if (!isset($result['error'])) {
                if ($this->getOnepage()->getQuote()->isVirtual()) {
                   ...
                   /* $result['goto_section'] = 'shipping_method';
					...
					Mage::log('Inside use_for_shipping', null, 'saveBillingAction.log');
