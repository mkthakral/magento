...
/     * Path to wsdl file of track service
    ...
    public function setRequest(Mage_Shipping_Model_Rate_Request $request)
    {
		...
		foreach ($request->getAllItems() as $quoteItem) {
			...
			//Mage::log(print_r($product->debug(), true), null, 'shipping.log');
			$_product = Mage::getModel('catalog/product')->load($id);
			//Mage::log(print_r($_product->debug(), true), null, 'shipping.log');
		}
		//Mage::log(print_r($request->debug(), true), null, 'shipping.log');
		
						$protable= Mage::getSingleton('core/resource')->getTableName('marketplace_product');
					    $sqlPaymentSystem1="SELECT * FROM ".$protable." WHERE mageproductid='".$id."'";
...
//Mage::log(print_r($artiscollection->debug(), true), null, 'shipping.log');
//echo "<pre>"; print_r($address);echo "</pre>";
 ...
 $country=$address['country_id'];
    ...
    protected function _prepareRateResponse($response)
    {
        ...
        if (empty($priceArr)) {
			//Mage::log('---No Results Available---', null, 'shipping.log');
			...
			<tr>
				<td>".reset($this->_request->getAllItems())->getName()."</td>
				<td>".reset($this->_request->getAllItems())->getProductId()."</td>
			</tr>
			</table>
			...
			";
			//Mage::log("Query product: ".print_r($this->_request->getAllItems()->debug(), true),null,'shipping.log');
			//Mage::log("Query product: ".print_r(reset($this->_request->getAllItems())->debug(), true),null,'shipping.log');
			//Mage::log("Namess: ".reset($this->_request->getAllItems())->getName(),null,'shipping.log');
			//Mage::log('Message: '.$message, null, 'shipping.log');
            ...
            foreach ($priceArr as $method=>$price) {
				Mage::log('+++Results Available+++:  '.count($result), null, 'shipping.log');
