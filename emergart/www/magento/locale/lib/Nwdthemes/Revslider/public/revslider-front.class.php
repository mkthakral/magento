...
class RevSliderFront extends RevSliderBaseFront{
	...
	public static function onAddScripts(){
        ...
        $do_inclusion = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_include_libraries', false);
		...
		Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-plugin-settings', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL .'public/assets/css/settings.css', array(), $slver);
		...
		Mage::helper('nwdrevslider/framework')->wp_enqueue_script(array('jquery'));
        ...
        Mage::helper('nwdrevslider/framework')->wp_enqueue_script('tp-tools', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL .'public/assets/js/jquery.themepunch.tools.min.js', $waitfor, $slver, $ft);
        Mage::helper('nwdrevslider/framework')->wp_enqueue_script('revmin', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL .'public/assets/js/jquery.themepunch.revolution.min.js', 'tp-tools', $slver, $ft);
		...
		Mage::helper('nwdrevslider/framework')->add_action('wp_footer', array('RevSliderFront', 'putAdminBarMenus'));
	...
	public static function add_admin_menu_nodes(){
		...
		if(!empty($sliders)){
			foreach($sliders as $id => $slider){
				self::_add_node('<span class="rs-label" data-alias="'.Mage::helper('nwdrevslider/framework')->esc_attr($slider['alias']).'">'.Mage::helper('nwdrevslider/framework')->esc_attr($slider['title']).'</span>', 'revslider', Mage::helper('nwdrevslider/framework')->admin_url('admin.php?page=revslider&view=slide&id=new&slider='.intval($id)), array('class' => 'revslider-sub-menu' ), Mage::helper('nwdrevslider/framework')->esc_attr($slider['alias'])); //<span class="wp-menu-image dashicons-before dashicons-update"></span>
			}
		}
	...
	public static function updateTables(){
		...
		if(version_compare($cur_ver, '1.0.1', '<')){ //add missing settings field, for new creates lines in slide editor for example
			//require_once(Nwdthemes_Revslider_Helper_Framework::ABSPATH . 'wp-admin/includes/upgrade.php');
