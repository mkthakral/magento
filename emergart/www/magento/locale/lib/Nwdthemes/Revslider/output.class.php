    ...
    private function putSlides($gal_ids = array(), $order = array()){
		...
		foreach($slides as $slide){
			...
			if($isExternal != 'external'){
				...
				if($img_id !== false){
					$img_data = Mage::helper('nwdrevslider/images')->wp_get_attachment_metadata( $img_id );
		...
		foreach($layers as $layer){
			...
			**/
			...
			switch($type){
				...
				case 'button':
					$html = $text;
					$html = Mage::helper('nwdrevslider/framework')->do_shortcode(stripslashes($html), $slide);
                    $html_toggle = $text_toggle;
                    $html_toggle = Mage::helper('nwdrevslider/framework')->do_shortcode(stripslashes($html_toggle), $slide);
					...
					foreach($icon_sets as $is){
						if(strpos($html, $is) !== false){ //include default Icon Sets if used
							...
							$$font_var = true;
							//Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-icon-set-'.$is);
						}
					}
				...
				case 'image':
					...
					$urlImage = Mage::helper('nwdrevslider/images')->imageUrl($urlImage);
                    ...
                    $urlImageData = explode('media/', $urlImage);
                    if (isset($urlImageData['1'])) {
                        $urlImage = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $urlImageData['1'];
                    }

					$img_w = '';
					$img_h = '';
					if($cur_img_id !== false){
						$img_data = Mage::helper('nwdrevslider/images')->wp_get_attachment_metadata( $cur_img_id );
						if($img_data !== false && !empty($img_data)){
							if($img_size !== 'full'){
								if(isset($img_data['sizes']) && isset($img_data['sizes'][$img_size])){
				...
				foreach($a_action as $num => $action){
					...
					switch($action){
						...
						case 'link':
							...
							//a_image_link
                            if(isset($a_image_link[$num])) $a_image_link[$num] = Mage::helper('nwdrevslider/framework')->do_shortcode($a_image_link[$num], $slide);
            ...
            Mage::helper('nwdrevslider/framework')->do_action('revslider_add_layer_attributes', $layer, $slide, $this->slider);
			echo $a_html;
			
            // SVG OUTPUT
            if (!empty($svg_val) && sizeof($svg_val)>0) {                
                echo '            data-svg_src="'.Mage::helper('nwdrevslider/images')->imageUrl($svg_val->{'src'}).'"'." \n";
            ...
            echo Mage::helper('nwdrevslider/framework')->apply_filters('revslider_layer_content', stripslashes($html), $html, $this->slider->getID(), $slide, $layer)." ";
            if ($toggle_allow!=false) {
                echo '</div>';
