...
class RevSliderSlide extends RevSliderElementsBase{
	...
	private function initByInstagram($sliderID){
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByInstagram_post', $this->postData, $sliderID, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
		...
		$text = str_replace(array('%name%', '{{name}}'), $attr['name'], $text);
		$text = str_replace(array('%price%', '{{price}}'), $attr['price'], $text);
		$text = str_replace(array('%special_price%', '{{special_price}}'), $attr['special_price'], $text);
		$text = str_replace(array('%short_description%', '{{short_description}}'), $attr['short_description'], $text);
		$text = str_replace(array('%description%', '{{description}}'), $attr['description'], $text);
		$text = str_replace(array('%view_link%', '{{view_link}}'), $attr['view_link'], $text);
		$text = str_replace(array('%cart_link%', '{{cart_link}}'), $attr['cart_link'], $text);
		$text = str_replace(array('%wishlist_link%', '{{wishlist_link}}'), $attr['wishlist_link'], $text);
