...
class RevSliderFunctionsWP {
	...
	public static function initStaticVars(){
		...
		/*self::$urlAdmin = Mage::helper('nwdrevslider/framework')->admin_url();
	...
	public static function isMultisite(){
		$isMultisite = Mage::helper('nwdrevslider/framework')->is_multisite();
		return($isMultisite);
	}
	...
	public static function getPathBase(){
		return Nwdthemes_Revslider_Helper_Framework::ABSPATH;
	}
	...
	public static function getPathUploads(){	
		return Mage::helper('nwdrevslider/images')->imageBaseDir();
	}
	...
	public static function getUrlUploads(){
		...
		if(self::isMultisite() == false){	//without multisite
			$baseUrl = Mage::helper('nwdrevslider/framework')->content_url()."/";
		}
		else{	//for multisite
			$arrUploadData = Mage::helper('nwdrevslider/framework')->wp_upload_dir();
    ...
    public static function isAdminUser(){
        return Mage::helper('nwdrevslider/framework')->current_user_can('administrator');
    }
	...
	public static function import_media($file_url, $folder_name) {
		//require_once(Nwdthemes_Revslider_Helper_Framework::ABSPATH . 'wp-admin/includes/image.php');
		
		$ul_dir = Mage::helper('nwdrevslider/framework')->wp_upload_dir();
		$artDir = '';
		...
		//$siteurl = Mage::helper('nwdrevslider/framework')->get_option('siteurl');

		if (@fclose(@fopen($file_url, "r")) && ! @is_dir($file_url)) { //make sure the file actually exists and not a folder
			...
			if($atc_id == false || $atc_id == NULL){
				...
				$artdata = array(
					'post_author' => 1, 
					'post_date' => Mage::helper('nwdrevslider/framework')->current_time('mysql'),
					'post_date_gmt' => Mage::helper('nwdrevslider/framework')->current_time('mysql'),
					'post_title' => $filename, 
					'post_status' => 'inherit',
					'comment_status' => 'closed',
					'ping_status' => 'closed',
					'post_name' => Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes(str_replace("_", "-", $filename)),
					'post_modified' => Mage::helper('nwdrevslider/framework')->current_time('mysql'),
					'post_modified_gmt' => Mage::helper('nwdrevslider/framework')->current_time('mysql'),
					'post_parent' => '',
				...
				$attach_id = Mage::helper('nwdrevslider/images')->wp_insert_attachment($artdata, $artDir.$folder_name.$filename);
			}else{
				$attach_id = $atc_id;
			}

			//generate metadata and thumbnails
			if($attach_data = Mage::helper('nwdrevslider/images')->wp_generate_attachment_metadata($attach_id, $saveDir)) Mage::helper('nwdrevslider/images')->wp_update_attachment_metadata($attach_id, $attach_data);
			if(!self::isMultisite()) $artDir = $artDir;
	...
	public static function registerWidget($widgetName){
		Mage::helper('nwdrevslider/framework')->add_action('widgets_init', create_function('', 'return register_widget("'.$widgetName.'");'));
	}
	...
	public static function getImageUrlFromPath($pathImage){
        ...
        if(strpos($pathLower, "http://") !== false || strpos($pathLower, "https://") !== false || strpos($pathLower, "www.") === 0 || empty($pathImage)) {
            $pathImage = Mage::helper('nwdrevslider/images')->imageFile($pathImage);
            return $pathImage;
        }
		
		$urlImage = self::getUrlUploads().$pathImage;
        $urlImage = Mage::helper('nwdrevslider/images')->imageFile($urlImage);
		return($urlImage); 
	}
	...
	public static function getCategoriesAssoc($taxonomy = "category"){
		...
		$cats = Mage::helper('nwdrevslider/framework')->get_categories($args);
		...
		foreach($cats as $cat){
			...
			$title = $cat->name; // TODO: Add products count . " ($numItems $itemsName)";
	...
	public static function getPostTypeTitle($postType){
		...
		$objType = Mage::helper('nwdrevslider/framework')->get_post_type_object($postType);
	...
	public static function getPostTypeTaxomonies($postType){
		$arrTaxonomies = Mage::helper('nwdrevslider/framework')->get_object_taxonomies(array( 'post_type' => $postType ), 'objects');
	...
	public static function getPostTypesAssoc($arrPutToTop = array()){
		 ...
		 $arrCustomTypes = Mage::helper('nwdrevslider/framework')->get_post_types(array('_builtin' => false));
    ...
    public static function getPostsByIDs($strIDs, $slider_id, $is_gal, $additional = array()){
		...
		$query = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_posts', $query, $slider_id);
		
		$arrPosts = Mage::helper('nwdrevslider/products')->getProductsByQuery($query);

		//Mage::helper('nwdrevslider/framework')->dmp($query);Mage::helper('nwdrevslider/framework')->dmp("num posts: ".count($arrPosts));exit();
	...
	public static function getPostCategories($postID,$arrTax){
		...
		$arrCats = Mage::helper('nwdrevslider/framework')->wp_get_post_terms( $postID, $arrTax);
	...
	public static function getPost($postID){
		$post = Mage::helper('nwdrevslider/framework')->get_post($postID);
		if(empty($post))
	...
	public static function getUrlPostImage($postID,$size = self::THUMB_FULL){
		...
		$post_thumbnail_id = Mage::helper('nwdrevslider/framework')->get_post_thumbnail_id( $postID );
		if(empty($post_thumbnail_id))
			return("");
		
		$arrImage = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($post_thumbnail_id,$size);
		if(empty($arrImage))
			return("");
	...
	public static function getPostThumbID($postID){
		$thumbID = Mage::helper('nwdrevslider/framework')->get_post_thumbnail_id( $postID );
		return($thumbID);
	}
	...
	public static function getAttachmentImage($thumbID,$size = self::THUMB_FULL){
		...
		$arrImage = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($thumbID,$size);
		if(empty($arrImage))
			return(false);
	...
	public static function getUserDisplayName($userID){
		...
		$displayName =  Mage::helper('nwdrevslider/framework')->get_the_author_meta('display_name', $userID);
	...
	public static function getCategoriesByIDs($arrIDs,$strTax = null){			
		...
		$arrCats = Mage::helper('nwdrevslider/framework')->get_categories( $args );
	...
	public static function getCategoriesHtmlList($catIDs,$strTax = null){
		...
		foreach ( $categories as $category ) {
			...
			$link = Mage::helper('nwdrevslider/framework')->get_category_link($catID);
			$catName = $category["name"];
			
			if(!empty($link))
				$thelist .= '<a href="' . Mage::helper('nwdrevslider/framework')->esc_url( $link ) . '" title="' . Mage::helper('nwdrevslider/framework')->esc_attr( sprintf( __( "View all posts in %s"), $category["name"] ) ) . '" ' . $rel . '>' . $catName.'</a>';
			else
				$thelist .= $catName;
	...
	public static function getTagsHtmlList($postID){
		$tagList = Mage::helper('nwdrevslider/framework')->get_the_tag_list("",",","",$postID);
		return($tagList);
	}
	...
	public static function convertPostDate($date, $with_time = false){
		if(empty($date))
			return($date);
		if($with_time){
			$date = Mage::helper('nwdrevslider/framework')->date_i18n(Mage::helper('nwdrevslider/framework')->get_option('date_format').' '.Mage::helper('nwdrevslider/framework')->get_option('time_format'), strtotime($date));
		}else{
			$date = Mage::helper('nwdrevslider/framework')->date_i18n(Mage::helper('nwdrevslider/framework')->get_option('date_format'), strtotime($date));
		}
	...
	public static function getPathContent(){		
		if(self::isMultisite()){
			...
			$pathContent = Nwdthemes_Revslider_Helper_Framework::WP_CONTENT_DIR;
    ...
    public static function update_option($handle, $value, $autoload = 'on'){ //on is on, false is 'off'
        ...
        if(!Mage::helper('nwdrevslider/framework')->add_option($handle, $value, '', $autoload)){ //returns false if option is not existing
            Mage::helper('nwdrevslider/framework')->delete_option($handle);
        }

        Mage::helper('nwdrevslider/framework')->add_option($handle, $value, '', $autoload);
    }
