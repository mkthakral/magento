<?php
  ...
  //Mage::app();
   ...
   // include Mage::getBaseDir('app').'/code/core/Mage/Usa/Model/Shipping/Carrier/Fedex-Emergart.php';
    //$request = Mage::getModel('shipping/rate_request');
    //Mage::getModel('Mage_Shipping_Model')->collectRates('ss');
...
$functions = new Mage_Shipping_Model_Rate_Request($artistCountry,$artistZIP,$destCountry,$destZIP,$productWeight,$artistCity,$destCity,$productHeight,$productWidth,$productLength,$productId,$productName);
$var = $functions->collectRates($functions);
//ar_dump(get_object_vars($var));
//print_r($var);
//Mage::log('Output: '.$var, null, 'shipping-fedex-rest.log');
//Mage::log('Output: '.$var, null, 'shipping-fedex-rest.log');
...
/     * Path to wsdl file of track service
    ...
    public function __construct($artistCountry,$artistZIP,$destCountry,$destZIP,$productWeight,$artistCity,$destCity,$productHeight,$productWidth,$productLength,$productId,$productName)
    {
//        parent::__construct();
...
/*
        foreach ($allowed_methods as $fedexShippingMethod){
            Mage::log('Shipping Method: '.$fedexShippingMethod, null, 'shipping-fedex-rest.log');
            $response = $this->_doRatesRequest($fedexShippingMethod);
            $preparedSmartpost = $this->_prepareRateResponse($response);
            $this->_result->append($preparedSmartpost);
        }
*/
...
Mage::log('Debug3', null, 'shipping-fedex-rest.log');
        ...
        if (empty($priceArr)) {
            Mage::log('Debug4', null, 'shipping-fedex-rest.log');
			...
			";
			//Mage::log("Query product: ".print_r($this->_request->getAllItems()->debug(), true),null,'shipping-fedex-rest.log');
			//Mage::log("Query product: ".print_r(reset($this->_request->getAllItems())->debug(), true),null,'shipping-fedex-rest.log');
			//Mage::log("Namess: ".reset($this->_request->getAllItems())->getName(),null,'shipping-fedex-rest.log');
