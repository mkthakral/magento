...
class RevSliderOperations extends RevSliderElementsBase{
	...
	public function getArrFontFamilys($slider = false){
		...
		/*if($slider !== false){
			...
			if(is_array($font_custom)){
				foreach($font_custom as $key => $curFont){
					...
					if($font != false)
						$font_custom[$key] = array('version' => __('Depricated Google Fonts'), 'label' => $font);
					else
		...
		foreach($googlefonts as $f => $val){
			$fonts[] = array('type' => 'googlefont', 'version' => __('Google Fonts'), 'label' => $f, 'variants' => $val['variants'], 'subsets' => $val['subsets']);
		}
        
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_operations_getArrFontFamilys', $fonts);
	}
	...
	public static function getStaticCss(){
		...
		$contentCSS = Mage::helper('nwdrevslider/framework')->get_option('revslider-static-css', '');
		...
		?>
			<html>
				<head>
					...
					$http = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? 'https' : 'http';
...
ob_end_clean();
	...
	public function checkPurchaseVerification($data){
		...
		$response = Mage::helper('nwdrevslider/framework')->wp_remote_post('http://updates.themepunch.tools/activate.php', array(
			'body' => array(
				'code' => urlencode($data['code']),
                'version' => urlencode(RevSliderGlobals::SLIDER_REVISION),
				'product' => Nwdthemes_Revslider_Helper_Data::REVSLIDER_PRODUCT
			)
		));
	...
	public function doPurchaseDeactivation($data){
		...
		$response = Mage::helper('nwdrevslider/framework')->wp_remote_post('http://updates.themepunch.tools/deactivate.php', array(
			'body' => array(
				'code' => urlencode($code),
				'product' => Nwdthemes_Revslider_Helper_Data::REVSLIDER_PRODUCT
			)
...
$presets[] = array (
	...
	public static function create_slider_page($added){
		...
		if ($content !== '') {
			...
			$_id = 'revslider-' . $page_id . '-' . Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($alias);
