...
class RevSliderTemplate {
	...
	public function _download_template($uid){
		...
		if(Mage::helper('nwdrevslider/filesystem')->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) { //check here to not flood the server
			$request = Mage::helper('nwdrevslider/framework')->wp_remote_post($this->templates_url.$this->templates_download, array(
				'body' => array(
					'code' => urlencode($code),
				...
				if($response = $request['body']) {
					if($response !== 'invalid'){
						...
						if($ret !== false){
                            ...
                            $return = array('error' => __('Can\'t write the file into the uploads folder "' . $upload_dir['basedir'].$this->templates_path . '", please change permissions and try again!', 'revslider'));
						}
                    }else{
            ...
            $return = array('error' => __('Can\'t write into the uploads folder "' . $upload_dir['basedir'].$this->templates_path . '", please change permissions and try again!', 'revslider'));
		}
	...
	public function _get_template_list($force = false){
		...
		if(time() - $last_check > 345600 || $force == true){ //4 days
			...
			$request = Mage::helper('nwdrevslider/framework')->wp_remote_post($this->templates_url.$this->templates_list, array(
				'body' => array(
					'code' => urlencode($code),
					'shop_version' => urlencode(self::SHOP_VERSION),
					'version' => urlencode(RevSliderGlobals::SLIDER_REVISION),
        ...
        if (isset($template["user_template"])) {
            ...
            <div class="template_thumb_more">
                ...
                if ($allow_install !== false) {
                    if($slider_id !== false){
                        ?>
                        <div class="install_template_slider" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Re-Install Slider'); ?></div>
                        <span class="tp-clearfix" style="margin-bottom:5px"></span>
                        <?php
                        if(isset($template['package']) && $template['package'] !== ''){
                            $txt = ($template['package_full_installded']) ? __('Re-Install Slider Pack') : __('Install Slider Pack');
                            ?>
