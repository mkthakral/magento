...
class RevSliderOutput {
	...
	public function putLayer($layer, $row_group_uid = false, $static_slide = false, $special_type = false){
		...
		$st_hover = array();
			...
			$bg_color = RevSliderFunctions::getVal($def_val_h, 'background-color', 'transparent');
                if($bg_color !== 'transparent'){
				$bg_trans = RevSliderFunctions::getVal($def_val_h, 'background-transparency', 1);
                    if($bg_trans > 0) $bg_trans *= 100;
                    $bg_trans = intval($bg_trans);
			...
			$border_color = RevSliderFunctions::getVal($def_val_h, 'border-color', 'transparent');
                if($border_color !== 'transparent'){
				$border_trans = RevSliderFunctions::getVal($def_val_h, 'border-transparency', 1);
                    if($border_trans > 0) $border_trans *= 100;
                    $border_trans = intval($border_trans);
	...
	public function set_placeholders($text){
		...
		$text = Mage::helper('nwdrevslider/framework')->apply_filters( 'revslider_gallery_set_placeholders', $text );
		return $text;
	}
...
<?php if($this->slider->getParam("slider_type") == "fullscreen"){ ?>
					e.fullScreenAutoWidth='<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("autowidth_force","off")); ?>';
					e.fullScreenAlignForce='<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("full_screen_align_force","off")); ?>';
					e.fullScreenOffsetContainer= '<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("fullscreen_offset_container","")); ?>';
					e.fullScreenOffset='<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("fullscreen_offset_size","")); ?>';
<?php } ?>
...
<?php } ?>
            ...
            if($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')){
				...
				if($enable_thumbnails == 'on'){
					...
					echo '								wrapper_color:"'. Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_wrapper_color','transparent')).'",'."\n";
					echo '								wrapper_opacity:"'. Mage::helper('nwdrevslider/framework')->esc_attr(round($this->slider->getParam('thumbnails_wrapper_opacity','100') / 100, 2)).'",'."\n";
					echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $thumbs_tmp);
					echo '\','."\n";
				...
				if($enable_tabs == 'on'){
					...
					echo '								wrapper_color:"'. Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_wrapper_color','transparent')).'",'."\n";
					echo '								wrapper_opacity:"'. Mage::helper('nwdrevslider/framework')->esc_attr(round($this->slider->getParam('tabs_wrapper_opacity','100') / 100, 2)).'",'."\n";
					echo '								tmp:\'';
					echo preg_replace( "/\r|\n/", "", $tabs_tmp);
					echo '\','."\n";
    ...
    public function putSliderBase($sliderID, $gal_ids = array(), $markup_export = false, $settings = array(), $order = array()){
		...
		try{
			...
			if($this->slider->getParam("js_to_body","false") == "true"){
				...
				if($enable_logs == 'on'){
					$urlIncludeJS = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/js/jquery.themepunch.enablelog.js?rev='. $slver;
					$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
				}

				$urlIncludeJS = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/js/jquery.themepunch.tools.min.js?rev='. $slver;
				$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
				$urlIncludeJS = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/js/jquery.themepunch.revolution.min.js?rev='. $slver;
				$htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
			...
			$progress_style = ' style="height: '.Mage::helper('nwdrevslider/framework')->esc_attr($progress_height).'px; background-color: '.RevSliderFunctions::hex2rgba($progressbar_color, $progress_opa).';"';
