...
class RevSliderObjectLibrary {
    ...
    public function _get_list($force = false){
        ...
        $last_check = Mage::helper('nwdrevslider/framework')->get_option('revslider-library-check');

        if($last_check == false){ //first time called
            $last_check = 1296001;
            Mage::helper('nwdrevslider/framework')->update_option('revslider-library-check',  time());
        }
        ...
        if(time() - $last_check > 1296000 || $force == true){ //30 days
            ...
            $request = Mage::helper('nwdrevslider/framework')->wp_remote_post($this->library_url.$this->library_list, array(
                'body' => $rattr
            ));
            ...
            if(!Mage::helper('nwdrevslider/framework')->is_wp_error($request)) {
                if($response = Mage::helper('nwdrevslider/framework')->maybe_unserialize($request['body'])) {
                    ...
                    if(is_array($library)) {
                        Mage::helper('nwdrevslider/framework')->update_option('rs-library', $library, false);
                    }
                }
            }
        }
    ...
    public function _get_object_thumb($object_handle, $type){
        ...
        if( ! file_exists($file) && Mage::helper('nwdrevslider/filesystem')->wp_mkdir_p($upload_dir['basedir'].$path)) {

            $curl = ($this->check_curl_connection()) ? $this->_curl : false;
            ...
            if(!file_exists($file) || isset($temp['push_image'])){
                ...
                if($curl !== false){
                    ...
                    if($validated == 'false' && $type != 'thumb'){
                        $error = __('Plugin not activated');
                    }else{
                        $code = ($validated == 'false') ? '' : Mage::helper('nwdrevslider/framework')->get_option('revslider-code', '');

                        $image_data = Mage::helper('nwdrevslider/framework')->wp_remote_post($this->library_url.$this->library_download, array(
                            'body' => array(
                                'code' => urlencode($code),
                        ...
                        if(!Mage::helper('nwdrevslider/framework')->is_wp_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
                            ...
                            $image_data = false;
                            $error = __('Error downloading object');
                        }
                    }
                }else{
                    //cant download file
    ...
    public function write_markup(){
        ...
        <div id="dialog_addobj" class="dialog-addobj" title="<?php echo Mage::helper('nwdrevslider')->__("Add Object Layer",'revslider'); ?>" style="display:none">
                ...
                <div id="addobj-list-of-items">
                    <div id="addobj-dialog-header">
                        <div class="object_library_search_wrapper">
                            <input type="text" id="obj_library_search" placeholder="<?php echo Mage::helper('nwdrevslider')->__("Search for Objects...",'revslider'); ?>" /><span id="obj_library_search_trigger"><i class="eg-icon-search"></i></span>
                        </div>
                        <div id="object_library_type_list_new">
                            <span id="obj_lib_main_cat_filt_all" data-value="all" class="obj_library_cats_filter"><?php echo Mage::helper('nwdrevslider')->__("ALL",'revslider'); ?></span>
                            <span id="obj_lib_main_cat_filt_allimages" data-value="allimages" class="obj_library_cats_filter all_img_cat"><?php echo Mage::helper('nwdrevslider')->__("ALL IMAGES",'revslider'); ?></span>
                            <span id="obj_lib_main_cat_filt_svg" data-value="svg" class="obj_library_cats_filter svg_cat"><?php echo Mage::helper('nwdrevslider')->__("SVG",'revslider'); ?></span>
                            <span id="obj_lib_main_cat_filt_icon" data-value="icon" class="obj_library_cats_filter fonticon_cat"><?php echo Mage::helper('nwdrevslider')->__("ICON",'revslider'); ?></span>
                            <span id="obj_lib_main_cat_filt_image" data-value="image" class="obj_library_cats_filter png_cat"><?php echo Mage::helper('nwdrevslider')->__("PNG",'revslider'); ?></span>
                            <span id="obj_lib_main_cat_filt_bgimage" data-value="bgimage" class="obj_library_cats_filter jpg_cat"><?php echo Mage::helper('nwdrevslider')->__("JPG",'revslider'); ?></span>
                        </div>
                        <div id="up-lic-ob-lib">
                            <div id="licence_obect_library"><i class="fa-icon-copyright"></i><?php echo Mage::helper('nwdrevslider')->__("License Info",'revslider'); ?></div>
                            <div id="update_obect_library"><i class="eg-icon-arrows-ccw"></i><?php echo Mage::helper('nwdrevslider')->__("Update Object Library",'revslider'); ?></div>
                        </div>
                        <div id="object-tag-list" class="object-tag-list"><span id="obj_library_cats_favorite" class="obj_library_cats" data-tag="favorite" data-image="true" data-bgimages="true" data-svg="true"><i class="fa-icon-star-o" style="margin-right: 5px;"></i><?php echo Mage::helper('nwdrevslider')->__("Favorite",'revslider'); ?></span><span id="obj_library_cats_allico" class="obj_library_cats" data-tag="allicon" data-icon="true" data-bgimages="false" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo Mage::helper('nwdrevslider')->__("All Icons",'revslider'); ?></span><span id="obj_library_cats_allpng" class="obj_library_cats" data-tag="allpng" data-image="true" data-bgimages="false" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo Mage::helper('nwdrevslider')->__("All PNG",'revslider'); ?></span><span id="obj_library_cats_alljpg" class="obj_library_cats" data-tag="alljpg" data-image="false" data-bgimages="true" data-svg="false"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo Mage::helper('nwdrevslider')->__("All JPG",'revslider'); ?></span><span id="obj_library_cats_allsvg" class="obj_library_cats" data-tag="allsvg" data-image="false" data-bgimages="false" data-svg="true"><i class="fa-icon-folder-o" style="margin-right: 5px;"></i><?php echo Mage::helper('nwdrevslider')->__("All SVG",'revslider'); ?></span></div>
                    </div>
            ...
            <div id="bg-vs-layer-wrapper">
                <span id="add_objimage_as_layer"><?php echo Mage::helper('nwdrevslider')->__("As Layer",'revslider'); ?></span><span class="addthisasbg" id="obj-layer-bg-switcher"></span><span id="add_objimage_as_slidebg"><?php echo Mage::helper('nwdrevslider')->__("As Slide BG",'revslider'); ?></span>
            </div>
            ...
            (function(jQuery) {
                ...
                jQuery('body').on('click', '.object_library_itemfavorit', function () {
                    ...
                    if (ic.hasClass("fa-icon-star")) {
                        ...
                        favoriteObjectsList = jQuery.grep(favoriteObjectsList, function (value) {
                            return value != d.src
                        });
                ...
                function loadObjNow(d) {
                    ...
                    } else if (ty === "img" || ty === "1" || ty === "2" || ty === 1 || ty === 2) {
                        if (src.indexOf("/") === -1) {
                ...
                function createFontIcons() {
                    ...
                    if (sheets)
                        jQuery.each(sheets, function (index, sheet) {
                            var found = false,
                                markup = "";
                            if (sheet && sheet.href && sheet.href.indexOf("revslider/public/assets/fonts/") >= 0) {
                                try {
                ...
                push_objects_to_library = function() {
                    var c = jQuery('#object_library_results-inner'),
                        svgi = 0,
                        pngi = 0,
                        jpgi = 0,
    ...
    public function load_objects(){
        ...
        $online = Mage::helper('nwdrevslider/framework')->get_option('rs-library', array());
        if(!empty($online)){
            $obj['online'] = $online;
        }
        ...
        if(file_exists($image_path) && $imgsize = getimagesize($image_path)) {
            ...
            foreach($sizes as $size){
                ...
                if(!file_exists($upload_directory.$modified_file_name_without_ending.'.'.$file_ending) || $force) {
                    ...
                    Mage::helper('nwdrevslider/images')->image_resize($image_path, $width, $height);
                }
            }
    ...
    public function get_correct_size_url($image_path, $imgres, $library_size = array()){
        ...
        if(!is_array($imgres)){
            ...
            $img_sizes = Mage::helper('nwdrevslider/framework')->get_intermediate_image_sizes();
    ...
    public function get_favorites(){
        return Mage::helper('nwdrevslider/framework')->get_option('rs_obj_favorites', array());
    }
    ...
    public function save_favorites($favourites){
        Mage::helper('nwdrevslider/framework')->update_option('rs_obj_favorites', $favourites);
    }
