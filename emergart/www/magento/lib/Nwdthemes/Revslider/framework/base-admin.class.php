...
class RevSliderBaseAdmin extends RevSliderBase {
	...
	public static function addAdminMenu(){
		...
		foreach(self::$arrMenuPages as $menu){
            ...
            $revslider_screens[] = Mage::helper('nwdrevslider/framework')->add_menu_page( $title, $title, $role, array(self::$t, $pageFunctionName), 'dashicons-update' );
		}
		...
		foreach(self::$arrSubMenuPages as $menu){
			...
			$revslider_screens[] = Mage::helper('nwdrevslider/framework')->add_submenu_page( 'revslider', $title, $title, $role, $pageSlug, array(self::$t, $pageFunctionName) );
		}
