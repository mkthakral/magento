...
class RevSliderOutput {
    ...
    private $storeID = 0;	//admin preview store id
    ...
    public static function putSlider($sliderID,$putIn="", $gal_ids = array(), $settings = array(), $order = array()){
        $settings = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_modify_slider_settings', $settings, $sliderID);
	 ...
	 * set preview mode
	 */
	public function setPreviewMode($storeID = 0){
	...
	private function filterOneSlide($slides){
        ...
        if(strpos($oneSlideID, 'static_') !== false){
            ...
            $tmp_slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".RevSliderGlobals::$table_static_slides." WHERE slider_id = %s", array($sliderID)), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
    ...
    private function putSlides($gal_ids = array(), $order = array()){
		...
		if($this->previewMode == true && $this->oneSlideMode == true){
            ...
            $slides = $this->slider->getSlidesForOutput($publishedOnly, $this->previewMode ? $this->storeID : Mage::app()->getStore()->getStoreId(), $this->sliderLang, $gal_ids);
        ...
        $mobile = (strstr($usragent,'Android') || strstr($usragent,'webOS') || strstr($usragent,'iPhone') ||strstr($usragent,'iPod') || strstr($usragent,'iPad') || strstr($usragent,'Windows Phone') || Mage::helper('nwdrevslider/framework')->wp_is_mobile()) ? true : false;
		...
		if(empty($slides)){
			...
			<div class="no-slides-text">
				...
				if($this->slider->isSlidesFromPosts()){
					echo Mage::helper('nwdrevslider')->__('No slides found, please add at least one Slide Template to the choosen language.');
				}else{
					echo Mage::helper('nwdrevslider')->__('No slides found, please add some slides');
				}
				?>
			</div>
			<?php
        ...
        $do_static = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_enable_static_layers', true);
		...
		foreach($slides as $slide){
			...
			if($this->previewMode === false){ // do only if we are not in preview mode
				$ts = Mage::helper('nwdrevslider/framework')->current_time('timestamp');
			...
			if($isExternal != 'external'){
				...
				switch($alt_type){
					case 'media_library':
						$alt = Mage::helper('nwdrevslider/framework')->get_post_meta( $img_id, '_wp_attachment_image_alt', true );
					break;
					case 'file_name':
					...
					break;
					case 'custom':
						$alt = Mage::helper('nwdrevslider/framework')->esc_attr($slide->getParam('alt_attr', ''));
					break;
				}

                switch($title_type){
                    case 'media_library':
                        $img_title = Mage::helper('nwdrevslider/framework')->get_the_title($img_id);
                    break;
                    case 'file_name':
                        $imageFilename = $slide->getImageFilename();
                        $info = pathinfo($imageFilename);
                    ...
                    break;
                    case 'custom':
                        $img_title = Mage::helper('nwdrevslider/framework')->esc_attr($slide->getParam('title_attr', ''));
                    break;
                }
				...
				if($img_id !== false){
					$img_data = Mage::helper('nwdrevslider/framework')->wp_get_attachment_metadata( $img_id );
                ...
                $urlSlideImage = Mage::helper('nwdrevslider/framework')->esc_url($slide->getParam('slide_bg_external', ''));
                $alt = Mage::helper('nwdrevslider/framework')->esc_attr($slide->getParam('alt_attr', ''));
                $img_title = Mage::helper('nwdrevslider/framework')->esc_attr($slide->getParam('title_attr', ''));
			...
			if($isThumbsActive == true || $is_special_nav == true){
				...
				if(empty($urlThumb)){
                    ...
                    // make sure it is url to file
                    $_pathinfo = pathinfo($urlThumb);
                    if ( ! isset($_pathinfo['extension']) || empty($_pathinfo['extension'])) {
                        $urlThumb = '';
                    }
                }
				...
				if($thumb_do == 'slider'){ //use the slider settings for width / height
					...
					if($source_type == 'youtube' || $source_type == 'vimeo' ||
						...
						if(empty($urlThumb)){	//try to get resized thumb
							...
							$urlThumb = Mage::helper('nwdrevslider/images')->rev_aq_resize($url_img_link, $th_width, $th_height, true, true, true);
						}else{
							$urlThumb = Mage::helper('nwdrevslider/images')->rev_aq_resize($urlThumb, $th_width, $th_height, true, true, true);
							if(empty($urlThumb)){
				...
				$urlThumb = $urlThumb ? : Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/images/transparent.png';

				$htmlThumb = ' data-thumb="'.Mage::helper('nwdrevslider/images')->imageUrl($urlThumb).'" ';
			...
			if($enableLink == 'true'){
				...
				switch($linkType){
					...
					default:
					case 'regular':
                        $link = Mage::helper('nwdrevslider/framework')->do_shortcode($slide->getParam('link', ''), $slide);
			...
			if(!empty($delay) && is_numeric($delay))
				$htmlDelay = ' data-delay="'. intval($delay).'" ';
			...
			$styleImage = '';
			$urlImageTransparent = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/images/transparent.png';
			...
			if($lazyLoad != 'none'){
				...
				$urlSlideImage = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/images/dummy.png';
			}
            ...
            $kb_blur_start = $slide->getParam('kb_blur_start', $this->slider->getParam('def-kb_blur_start', '0'));
            $kb_blur_end = $slide->getParam('kb_blur_end', $this->slider->getParam('def-kb_blur_end', '0'));
			...
			if($kenburn_effect == "on" && ($bgType == 'image' || $bgType == 'external')){
				...
				$kb_pz .= ' data-blurstart="'.$kb_blur_start.'"';
				$kb_pz .= ' data-blurend="'.$kb_blur_end.'"';
			...
			if($post_based_slider){//check if we are post based or normal slider
				$the_post = Mage::helper('nwdrevslider/framework')->get_post($slide->getID());
				$new_title = @Mage::helper('nwdrevslider/framework')->get_the_title($slide->getID());
				$the_excerpt = strip_tags(Mage::helper('nwdrevslider/framework')->strip_shortcodes($the_post->post_excerpt));

				$slide_title = ' data-title="'.stripslashes(Mage::helper('nwdrevslider/framework')->esc_attr($new_title)).'"';
				$slide_description = ' data-description="'.str_replace(array("\'", '\"'), array("'", '"'), Mage::helper('nwdrevslider/framework')->esc_attr($the_excerpt)).'"';
			}else{
				$slide_title = ' data-title="'.stripslashes(Mage::helper('nwdrevslider/framework')->esc_attr($slide->getParam("title","Slide"))).'"';
				$slide_description = ' data-description="'.str_replace(array("\'", '\"'), array("'", '"'), Mage::helper('nwdrevslider/framework')->esc_attr($slide->getParam('slide_description', ''))).'"';
			}
			...
			for($mi=1;$mi<=10;$mi++){
                ...
                $add_params .= ' data-param'.$mi.'="'.stripslashes(Mage::helper('nwdrevslider/framework')->esc_attr($pa)).'"';
			}
            ...
            Mage::helper('nwdrevslider/framework')->do_action('revslider_add_li_data', $this->slider, $slide);
			...
			Mage::helper('nwdrevslider/framework')->do_action('revslider_add_layer_html', $this->slider, $slide);
        ...
        if($do_static && !empty($this->static_slide)){
            ...
            if(!empty($layers)){
                ...
                Mage::helper('nwdrevslider/framework')->do_action('revslider_add_static_layer_html', $this->slider);
		...
		switch($videoType){
			...
			case 'twitter':
				...
				if($youtube_id !== ''){
					...
					$arguments.=';origin=' . rtrim(Mage::getBaseUrl(), '/') . ';';
			...
			case 'html5':
				...
				$html_mpeg = Mage::helper('nwdrevslider/images')->imageUrl( $slide->getParam('slide_bg_html_mpeg', '') );
				$html_webm = Mage::helper('nwdrevslider/images')->imageUrl( $slide->getParam('slide_bg_html_webm', '') );
				$html_ogv =  Mage::helper('nwdrevslider/images')->imageUrl( $slide->getParam('slide_bg_html_ogv', '')  );
	...
	private function putCreativeLayer($static_slide = false){
		...
		$layers = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_putCreativeLayer', $this->layers, $this, $static_slide);
	...
	public function putLayer($layer, $row_group_uid = false, $static_slide = false, $special_type = false){
		$layer = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_putLayer_pre', $layer, $this, $row_group_uid, $static_slide, $special_type);
		...
		$tabindex = RevSliderFunctions::getVal($layer, 'attrTabindex');
		...
		$blur_write_all = ($blurfilter != 0 && $blurfilter != '0px' ||
							...
							$anim_blurfilter_start != 0 && $anim_blurfilter_start != '0px' ||
							$anim_blurfilter_end != 0 && $anim_blurfilter_end != '0px') ? true : false;

		$grayscale_write_all = ($grayscalefilter != 0 && $grayscalefilter != '0%' ||
        ...
        $dbot = false;
			...
			**/
			$do_remove_inline = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_remove_inline', true);
			...
			if($do_remove_inline){
                if(isset($this->in_class_usage[trim($class)]) && isset($this->in_class_usage[trim($class)]['params'])){//defaults get set here
                    ...
                    $dls = (isset($this->in_class_usage[trim($class)]['params']->{'letter-spacing'})) ? $this->in_class_usage[trim($class)]['params']->{'letter-spacing'} : $dls;
                    ...
                    $dco = $dco!==false ? $dcot!==false ? $dco = TPColorpicker::convert($dco,$dcot) : $dco = TPColorpicker::get($dco) : $dco;
                    ...
                    $dbc = $dbc!==false ? $dbt!==false ? $dbc = TPColorpicker::convert($dbc,$dbt) : $dbc = TPColorpicker::get($dbc) : $dbc;
                    ...
                    $dboc = $dboc!==false ? $dbot!==false ? $dboc = TPColorpicker::convert($dboc,$dbot) : $dboc = TPColorpicker::get($dboc) : $dboc;
		...
		if($anim_blurfilter_start != 0 && $anim_blurfilter_start != '0px' || $blur_write_all){
			$tcin .= 'fb:'.$anim_blurfilter_start.';';
		}
		...
		$splitout = RevSliderFunctions::getVal($frame_end, 'split', RevSliderFunctions::getVal($layer, 'endsplit', 'none'));
		$splitindir = RevSliderFunctions::getVal($frame_start, 'split_direction', RevSliderFunctions::getVal($layer, 'split_direction', 'forward'));
		$splitoutdir = RevSliderFunctions::getVal($frame_end, 'split_direction', RevSliderFunctions::getVal($layer, 'endsplit_direction', 'forward'));
		...
		$elementdelay = round($elementdelay,4);
		$endelementdelay = round($endelementdelay,4);
		...
		switch($type){
				...
				case 'button':
					$html = $text;
					$html = Mage::helper('nwdrevslider/framework')->do_shortcode(stripslashes($html));
                    $html_toggle = $text_toggle;
                    $html_toggle = Mage::helper('nwdrevslider/framework')->do_shortcode(stripslashes($html_toggle));
					...
					if(!empty($static_styles)){
						...
						if(!empty($static_styles['font-size'])){
							...
							if ( ! empty($mcfs) && $mcfs !== $dfos) $inline_styles .= ' font-size: '.$mcfs.';';
						}
						if(!empty($static_styles['line-height'])){
							...
							if ( ! empty($mcfs) && $mclh !== $dlh) $inline_styles .= ' line-height: '.$mclh.';';
						}
						...
						if(!empty($static_styles['color'])) {
                            ...
                            $def_val = (array)RevSliderFunctions::getVal($layer, 'deformation', array());
                            $color_trans = RevSliderFunctions::getVal($def_val, 'color-transparency', false);
                            $use_color = $use_color !== false ? $color_trans !== false ? $use_color = TPColorpicker::convert($use_color, $color_trans) : $use_color = TPColorpicker::get($use_color) : $use_color;

                            if ($use_color != $dco)
                                $inline_styles .= ' color: ' . $use_color . ';';
                        }
                        if(!empty($static_styles['letter-spacing'])){
                            $static_styles['letter-spacing'] = RevSliderFunctions::add_missing_val($static_styles['letter-spacing'], 'px');
                            if(is_object($static_styles['letter-spacing'])){
                                $mcls = RevSliderFunctions::get_biggest_device_setting($static_styles['letter-spacing'], $this->enabled_sizes);
                            }else{
                                $mcls = $static_styles['letter-spacing'];
                            }
                            if( ! empty($mcls) && $mcls !== $dls) $inline_styles .= ' letter-spacing: '.$mcls.';';
                        }
				...
				case 'image':
					...
					if($img_size !== 'full'){
						if($cur_img_id !== false && !empty($cur_img_id)){
							$urlImage = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($cur_img_id, $img_size);
							$urlImage = $urlImage[0];
						}
					}
					...
					if($cur_img_id !== false && !empty($cur_img_id)){
						$img_data = Mage::helper('nwdrevslider/framework')->wp_get_attachment_metadata( $cur_img_id );
						if($img_data !== false && !empty($img_data)){
							if($img_size !== 'full'){
								if(isset($img_data['sizes']) && isset($img_data['sizes'][$img_size])){
					...
					switch($alt_option){
						case 'media_library':
							if($cur_img_id !== false){
								$alt = Mage::helper('nwdrevslider/framework')->get_post_meta( $cur_img_id, '_wp_attachment_image_alt', true );
							}
							break;
						case 'file_name':
					...
					if($lazyLoad != 'none' || $do_ll == 'force' && $do_ll !== 'ignore'){
						...
						if($seo_opti === 'false' || $seo_opti === false){
							...
							$urlImage = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL."public/assets/images/dummy.png";
						}else{
                ...
                case 'audio':
                    ...
                    $urlAudio = Mage::helper('nwdrevslider/framework')->esc_attr(RevSliderFunctions::getVal($videoData, 'urlAudio'));
                    if(!empty($urlAudio)) $add_data .= ' data-videomp4="'. Mage::helper('nwdrevslider/images')->imageUrl($urlAudio).'"';
				...
				case 'video':
					...
					switch($videoType){
                        ...
                        case 'streaminstagramboth':
                        case 'html5':
                            $urlPoster = Mage::helper('nwdrevslider/images')->imageUrl(RevSliderFunctions::getVal($videoData, "urlPoster"));
                            $urlMp4 = Mage::helper('nwdrevslider/images')->imageUrl(RevSliderFunctions::getVal($videoData, "urlMp4"));
                            $urlWebm = Mage::helper('nwdrevslider/images')->imageUrl(RevSliderFunctions::getVal($videoData, "urlWebm"));
                            $urlOgv = Mage::helper('nwdrevslider/images')->imageUrl(RevSliderFunctions::getVal($videoData, "urlOgv"));
					...
					$videoThumbnail = (isset($videoData["previewimage"])) ? Mage::helper('nwdrevslider/images')->imageUrl($videoData["previewimage"]) : '';
			...
			case 'column':
				...
				$add_data .= ' data-columnwidth="'.Mage::helper('nwdrevslider/framework')->esc_attr($size).'%"'."\n";
		...
		switch($this->container_mode){
			...
			if($has_trigger){
				...
				$last_trigger_state = '			data-lasttriggerstate="'.Mage::helper('nwdrevslider/framework')->esc_attr($trigger_memory).'"';
			...
			if($animation_overwrite == 'waitout' || $animation_overwrite == 'wait'){
				...
				if($version === false || intval($version) < 530) {
					...
					$realEndTime = RevSliderFunctions::getVal($layer, "realEndTime", false);

					if($realEndTime !== false){
						$endSpeed = trim(RevSliderFunctions::getVal($frame_end, 'speed', RevSliderFunctions::getVal($layer, 'endspeed')));
		...
		if($anim_blurfilter_end != 0 && $anim_blurfilter_end != '0px' || $blur_write_all){
			$tcout .= 'fb:'.$anim_blurfilter_end.';';
		}
		...
		if($this->adv_resp_sizes == true){
				...
				if(!empty($static_styles)){
					...
					if(!empty($static_styles['color'])){
						if(is_object($static_styles['color'])){
							...
							foreach($static_styles['color'] as $sk => $sv){
                                $color_trans = RevSliderFunctions::getVal($def_val, 'color-transparency', false);
                                $static_styles['color']->$sk = TPColorpicker::convert($sv, $color_trans);
							}
                    ...
                    if(!empty($static_styles['letter-spacing'])){
                        if(is_object($static_styles['letter-spacing'])){
                            $ss_ls = RevSliderFunctions::normalize_device_settings($static_styles['letter-spacing'], $this->enabled_sizes, 'html-array');
                            if(strpos($ss_ls, '[') !== false) $static_data .= str_replace('px', '', '			data-letterspacing="'.$ss_ls.'"')."\n";
                        }
                    }
				}
        ...
        $calc_bg_color = TPColorpicker::convert($bg_color, $bg_trans);
        $calc_dbc_color = TPColorpicker::convert($dbc, $dbt);

        if (strpos($calc_bg_color,"gradient")!==false || strpos($calc_dbc_color,"gradient")!==false)
            $st_idle['background'] = array(TPColorpicker::convert($bg_color, $bg_trans), TPColorpicker::convert($dbc, $dbt)); //'ALWAYS'

        else
            $st_idle['background-color'] = array(TPColorpicker::convert($bg_color, $bg_trans), TPColorpicker::convert($dbc, $dbt)); //'ALWAYS'
        ...
        $dbot = intval($dbot);
        $st_idle['border-color'] = array(TPColorpicker::convert($border_color, $border_trans), TPColorpicker::convert($dboc, $dbot)); //'ALWAYS'
        //}
        ...
        if($is_hover_active){
            ...
            $frame_hover['ease'] = RevSliderFunctions::getVal($def_val_h, 'easing', 'easeOutExpo');
            ...
            if($font_color !== 'transparent'){
                $font_trans = RevSliderFunctions::getVal($def_val_h, 'color-transparency', false);
                $st_hover['c'] = array(TPColorpicker::convert($font_color, $font_trans), 'ALWAYS');
            }else{
                $st_hover['c'] = array($font_color, 'ALWAYS');
            }
            $mfs = RevSliderFunctions::getVal($def_val_h, 'font-style', 'off');
            $font_style = ($mfs == 'on' || $mfs == 'italic') ? 'italic' : 'normal';
            $st_hover['fs'] = array($font_style, 'normal');
            $st_hover['td'] = array(RevSliderFunctions::getVal($def_val_h, 'text-decoration', 'none'), 'none');
            $bg_color = RevSliderFunctions::getVal($def_val_h, 'background-color', 'transparent');
            //if($bg_color !== 'transparent'){
            $bg_trans = RevSliderFunctions::getVal($def_val_h, 'background-transparency', false);
            $st_hover['bg'] = array(TPColorpicker::convert($bg_color, $bg_trans), 'ALWAYS');
            //}

            $st_hover['zi'] = array(RevSliderFunctions::getVal($def_val_h, 'zindex', 'auto'), 'auto');

            $border_color = RevSliderFunctions::getVal($def_val_h, 'border-color', 'transparent');
            //if($border_color !== 'transparent'){
            $border_trans = RevSliderFunctions::getVal($def_val_h, 'border-transparency', false);
            $st_hover['bc'] = array(TPColorpicker::convert($border_color, $border_trans), 'ALWAYS');
            //}

            $st_hover['bs'] = array(RevSliderFunctions::getVal($def_val_h, 'border-style', 'none'), 'none');
            $my_border_width = RevSliderFunctions::getVal($def_val_h, 'border-width', array('0px','0px','0px','0px'));
            ...
            $my_border = RevSliderFunctions::getVal($def_val_h, 'border-radius', array('0px','0px','0px','0px'));
            if(!empty($my_border)){
                $my_border = implode(' ', $my_border);
		...
		$a_action_speed = RevSliderFunctions::cleanStdClassToArray(RevSliderFunctions::getVal($all_actions, 'action_speed', array()));
		$a_action_easing = RevSliderFunctions::cleanStdClassToArray(RevSliderFunctions::getVal($all_actions, 'action_easing', array()));
            $a_link_type = RevSliderFunctions::cleanStdClassToArray(RevSliderFunctions::getVal($all_actions, 'link_type', array()));
		$a_link_follow = RevSliderFunctions::cleanStdClassToArray(RevSliderFunctions::getVal($all_actions, 'link_follow', array()));
                ...
                foreach($a_action as $num => $action){
                    ...
                    switch($action){
                        ...
                        case 'link':
                            ...
                            //a_image_link
                            if(isset($a_image_link[$num])) $a_image_link[$num] = Mage::helper('nwdrevslider/framework')->do_shortcode($a_image_link[$num]);
                            if(isset($a_link_type[$num]) && $a_link_type[$num] == 'jquery'){
								...
								if(isset($a_link_follow[$num]) && $a_link_follow[$num] == "nofollow" ){
									$html_simple_link .=' rel="nofollow"';
								}

								//write textdecoration always, even if none
								$def_val = (array) RevSliderFunctions::getVal($layer, 'deformation', array());
								$te_de = RevSliderFunctions::getVal($def_val, 'text-decoration', 'none');
								if($dtd === $te_de){ //was not written before, so write it now
									$idle_style .= 'text-decoration: '.$te_de.';';
								}
								$te_de_h = RevSliderFunctions::getVal($def_val_h, 'text-decoration', 'none');
								if($te_de_h === 'none'){
									$st_h_string .= 'text-decoration: '.$te_de_h.';';
								}
								}
							}
						break;
						...
						case 'scroll_under': //ok
                            ...
                            $a_action_delay[$num] = (isset($a_action_delay[$num])) ? $a_action_delay[$num] : '';
						$a_action_speed[$num] = (isset($a_action_speed[$num])) ? $a_action_speed[$num] : '300';
						$a_action_easing[$num] = (isset($a_action_easing[$num])) ? $a_action_easing[$num] : 'Linear.easeNone';
							$a_events[] = array(
								'event' => $a_tooltip_event[$num],
								'action' => 'scrollbelow',
                                'offset' => RevSliderFunctions::add_missing_val($a_scrolloffset[$num], 'px'),
							'delay' => $a_action_delay[$num],
							'speed' => $a_action_speed[$num],
							'ease' => $a_action_easing[$num]
							);
						break;
						case 'start_in':
							$a_tooltip_event[$num] = (isset($a_tooltip_event[$num])) ? $a_tooltip_event[$num] : '';
					...
					// Filter the Actions
					$a_events = Mage::helper('nwdrevslider/framework')->apply_filters( 'rs_action_output_layer_action', $a_events, $action, $all_actions, $num, $this->slide);
					if(!isset($html_simple_link)){
						$html_simple_link = "";
					}
					$html_simple_link = Mage::helper('nwdrevslider/framework')->apply_filters('rs_action_output_layer_simple_link', $html_simple_link, $action, $all_actions, $num, $this->slide, $this->slider, $a_events);
		...
		echo ($tabindex != '') ? '			'.$tabindex." \n" : '';
			...
			echo '            data-type="'.Mage::helper('nwdrevslider/framework')->esc_attr($type).'"'." \n";
			...
			Mage::helper('nwdrevslider/framework')->do_action('revslider_add_layer_attributes', $layer, $this->slide, $this->slider);

			echo $a_html;

            // SVG OUTPUT
			if (!empty($svg_val) && is_array($svg_val) && sizeof($svg_val)>0){
				echo '            data-svg_src="'.Mage::helper('nwdrevslider')->convertAssetUrlForOutput($svg_val['src']).'"'." \n";
			...
			if($splitin !== 'none'){
			$frame_start['splitdelay'] = round($elementdelay,4);
			}
			if($splitout !== 'none'){
			$frame_end['splitdelay'] = round($endelementdelay,4);
			}
		...
		if(!empty($frame_start)){
			...
			if(empty($frame_start['split']) || $frame_start['split'] == 'none'){
                ...
                unset($frame_start['split_direction']);
			}
			if(empty($frame_end['split']) || $frame_end['split'] == 'none'){
				unset($frame_end['split']);
				unset($frame_end['splitdelay']);
                unset($frame_end['split_direction']);
            }

            if(empty($frame_start['sfx_effect']) || $frame_start['sfx_effect'] == 'none'){
                unset($frame_start['sfx_effect']);
                unset($frame_start['sfxcolor']);
            }

            if(empty($frame_end['sfx_effect']) || $frame_end['sfx_effect'] == 'none'){
		...
		if($special_type === false){
			echo Mage::helper('nwdrevslider/framework')->apply_filters('revslider_layer_content', stripslashes($html), $html, $this->slider->getID(), $this->slide, $layer)." ";
		}
		if ($toggle_allow!=false) {
			echo '</div>';
			echo '<div class="';
	...
	public function set_placeholders($text){
		global $post; // TODO: NWD: Get rid of globals
		if(isset($post->ID)) $text = str_replace(array('%current_page_link%', '{{current_page_link}}'), Mage::helper('nwdrevslider/framework')->get_permalink($post->ID), $text);
		$text = str_replace(array('%home_url%', '{{home_url}}'), Mage::helper('nwdrevslider/framework')->esc_url( Mage::helper('nwdrevslider/framework')->home_url( '/' ) ), $text);
        if(isset($post->ID)) $text = Mage::helper('nwdrevslider/framework')->apply_filters( 'revslider_gallery_set_placeholders', $text , $post->ID );
		return $text;
	}
	...
	private function putJS($markup_export = false){
		...
		$do_delay = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_add_js_delay', $do_delay);
		...
		<script type="text/javascript">
            ...
            if (_RSdisableOnMobile && _RSisMobile) {
                var _RSelement = document.getElementById('<?php echo $this->sliderHtmlID; ?>');
                _RSelement.outerHTML = "";
                delete _RSelement;
            } else {

                <?php echo '(function(jQuery) {' . "\n"; ?>
            ...
            echo "{c: '#". $this->sliderHtmlID ."',";
                   ...
                   if ($this->slider->getParam("slider_type") == "fullscreen") {
                       echo ",";
                       echo " fullScreenAutoWidth:'" . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("autowidth_force", "off")) . "',";
                       echo " fullScreenAlignForce:'" . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("full_screen_align_force", "off")) . "',";
                       echo " fullScreenOffsetContainer:'" . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("fullscreen_offset_container", "")) . "',";
                       echo " fullScreenOffset:'" . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("fullscreen_offset_size", "")) . "'";
                   }
...
<?php
               ...
               if ($do_delay > 0) {
                   echo '						startDelay: ' . Mage::helper('nwdrevslider/framework')->esc_attr($do_delay) . ',' . "\n";
               }

               echo '						sliderType:"' . Mage::helper('nwdrevslider/framework')->esc_attr($slider_type) . '",' . "\n";

               echo 'jsFileLocation:"' . Mage::helper('nwdrevslider/framework')->esc_attr(Mage::getDesign()->getSkinUrl('nwdthemes/revslider/public/assets/js/')) . '",' . "\n";
               ...
               echo '						sliderLayout:"' . $sl_layout . '",' . "\n";
               echo '						dottedOverlay:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("background_dotted_overlay", "none")) . '",' . "\n";
               echo '						delay:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("delay", "9000", RevSlider::FORCE_NUMERIC)) . ',' . "\n";

               Mage::helper('nwdrevslider/framework')->do_action('revslider_fe_javascript_option_output', $this->slider);
               ...
               if ($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')) {
                   echo '						navigation: {' . "\n";
                   echo '							keyboardNavigation:"' . Mage::helper('nwdrevslider/framework')->esc_attr($keyboard_enabled) . '",' . "\n";
                   echo '							keyboard_direction: "' . Mage::helper('nwdrevslider/framework')->esc_attr($keyboard_direction) . '",' . "\n";
                   echo '							mouseScrollNavigation:"' . Mage::helper('nwdrevslider/framework')->esc_attr($mousescroll_enabled) . '",' . "\n";
                   echo '                             mouseScrollReverse:"' . Mage::helper('nwdrevslider/framework')->esc_attr($mousescroll_reverse) . '",' . "\n";

                   if ($slider_type !== 'hero')
                       echo '							onHoverStop:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("stop_on_hover", "on")) . '",' . "\n";

                   $add_comma = false;
                   if ($touch_enabled == 'on') {
                       $add_comma = true;
                       echo '							touch:{' . "\n";
                       echo '								touchenabled:"' . Mage::helper('nwdrevslider/framework')->esc_attr($touch_enabled) . '",' . "\n";
                       echo '                              touchOnDesktop:"' . Mage::helper('nwdrevslider/framework')->esc_attr($touch_enabled_desktop) . '",' . "\n";
                       echo '								swipe_threshold: ' . Mage::helper('nwdrevslider/framework')->esc_attr($swipe_velocity) . ',' . "\n";
                       echo '								swipe_min_touches: ' . Mage::helper('nwdrevslider/framework')->esc_attr($swipe_min_touches) . ',' . "\n";
                       echo '								swipe_direction: "' . Mage::helper('nwdrevslider/framework')->esc_attr($swipe_direction) . '",' . "\n";
                   ...
                   if ($enable_arrows == 'on') {
                       ...
                       $add_comma = true;
                       echo '							arrows: {' . "\n";
                       echo '								style:"' . Mage::helper('nwdrevslider/framework')->esc_attr($navigation_arrow_style) . '",' . "\n";
                       echo '								enable:';
                       ...
                       if ($hide_arrows_on_mobile === 'true') {
                           echo '								hide_under:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('arrows_under_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       if ($hide_arrows_over === 'true') {
                           echo '								hide_over:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('arrows_over_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								hide_onleave:' . $arrows_always_on . ',' . "\n";
                       if ($arrows_always_on === 'true') {
                           echo '								hide_delay:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_arrows', '200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                           echo '								hide_delay_mobile:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_arrows_mobile', '1200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								tmp:\'';
                       echo preg_replace("/\r|\n/", "", $arr_tmp);
                       echo '\',' . "\n";
                       ...
                       echo '									h_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('leftarrow_align_hor', 'left')) . '",' . "\n";
                       echo '									v_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('leftarrow_align_vert', 'center')) . '",' . "\n";
                       echo '									h_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('leftarrow_offset_hor', '20', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                                    v_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('leftarrow_offset_vert', '0', RevSlider::FORCE_NUMERIC)) . "\n";
                       echo '								},' . "\n";
                       echo '								right: {' . "\n";
                       echo (in_array($this->slider->getParam('rightarrow_position', 'slider'), array('layergrid', 'grid'))) ? '                                    container:"layergrid",' . "\n" : '';
                       echo '									h_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('rightarrow_align_hor', 'right')) . '",' . "\n";
                       echo '									v_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('rightarrow_align_vert', 'center')) . '",' . "\n";
                       echo '									h_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('rightarrow_offset_hor', '20', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                                    v_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('rightarrow_offset_vert', '0', RevSlider::FORCE_NUMERIC)) . "\n";
                       echo '								}' . "\n";
                       echo '							}' . "\n";
                   }
                   ...
                   if ($enable_bullets == 'on') {
                       ...
                       if ($hide_bullets_on_mobile === 'true') {
                           echo '								hide_under:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_under_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       if ($hide_bullets_over === 'true') {
                           echo '								hide_over:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_over_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								style:"' . Mage::helper('nwdrevslider/framework')->esc_attr($navigation_bullets_style) . '",' . "\n";
                       echo '								hide_onleave:' . $bullets_always_on . ',' . "\n";
                       if ($bullets_always_on === 'true') {
                           echo '								hide_delay:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_bullets', '200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                           echo '								hide_delay_mobile:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_bullets_mobile', '1200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								direction:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_direction', 'horizontal')) . '",' . "\n";
                       echo (in_array($this->slider->getParam('bullets_position', 'slider'), array('layergrid', 'grid'))) ? '                                    container:"layergrid",' . "\n" : '';
                       echo '								h_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_align_hor', 'right')) . '",' . "\n";
                       echo '								v_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_align_vert', 'center')) . '",' . "\n";
                       echo '								h_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_offset_hor', '20', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								v_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_offset_vert', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                                space:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bullets_space', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								tmp:\'';
                       echo preg_replace("/\r|\n/", "", $bul_tmp);
                       echo '\'' . "\n";
                       echo '							}' . "\n";
                   ...
                   if ($enable_thumbnails == 'on') {
                       ...
                       $add_comma = true;
                       echo '							thumbnails: {' . "\n";
                       echo '								style:"' . Mage::helper('nwdrevslider/framework')->esc_attr($thumbnails_style) . '",' . "\n";
                       echo '								enable:';
                       echo ($this->slider->getParam('enable_thumbnails', 'off') == 'on') ? 'true' : 'false';
                       echo ',' . "\n";
                       echo ($this->slider->getParam('rtl_thumbnails', 'off') == 'on') ? '                                rtl:true,' . "\n" : '';
                       echo '								width:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumb_width', '100', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								height:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumb_height', '50', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								min_width:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumb_width_min', '100', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								wrapper_padding:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_padding', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                              wrapper_color:"' . TPColorpicker::convert(Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_wrapper_color', 'transparent')), $this->slider->getParam('thumbnails_wrapper_opacity', false)) . '",' . "\n";
                       echo '								tmp:\'';
                       echo preg_replace("/\r|\n/", "", $thumbs_tmp);
                       echo '\',' . "\n";
                       echo '								visibleAmount:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumb_amount', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								hide_onmobile:' . $hide_thumbs_on_mobile . ',' . "\n";
                       if ($hide_thumbs_on_mobile === 'true') {
                           echo '								hide_under:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbs_under_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       if ($hide_thumbs_over === 'true') {
                           echo '								hide_over:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbs_over_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								hide_onleave:' . $thumbs_always_on . ',' . "\n";
                       if ($thumbs_always_on === 'true') {
                           echo '								hide_delay:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_thumbs', '200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                           echo '								hide_delay_mobile:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_thumbs_mobile', '1200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								direction:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnail_direction', 'horizontal')) . '",' . "\n";
                       echo '								span:';
                       echo ($this->slider->getParam('span_thumbnails_wrapper', 'off') == 'on') ? 'true' : 'false';
                       echo ',' . "\n";
                       echo '								position:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_inner_outer', 'inner')) . '",' . "\n";
                       ...
                       echo '								space:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_space', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								h_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_align_hor', 'left')) . '",' . "\n";
                       echo '								v_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_align_vert', 'center')) . '",' . "\n";
                       echo '								h_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_offset_hor', '20', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                                v_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('thumbnails_offset_vert', '0', RevSlider::FORCE_NUMERIC)) . "\n";
                       echo '							}' . "\n";
                   }
                   if ($enable_tabs == 'on') {
                       ...
                       $add_comma = true;
                       echo '							tabs: {' . "\n";
                       echo '								style:"' . Mage::helper('nwdrevslider/framework')->esc_attr($tabs_style) . '",' . "\n";
                       echo '								enable:';
                       echo ($this->slider->getParam('enable_tabs', 'off') == 'on') ? 'true' : 'false';
                       echo ',' . "\n";
                       echo ($this->slider->getParam('rtl_tabs', 'off') == 'on') ? '                                rtl:true,' . "\n" : '';
                       echo '								width:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_width', '100', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								height:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_height', '50', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								min_width:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_width_min', '100', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								wrapper_padding:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_padding', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                              wrapper_color:"' . TPColorpicker::convert(Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_wrapper_color', 'transparent')), $this->slider->getParam('tabs_wrapper_opacity', false)) . '",' . "\n";
                       echo '								tmp:\'';
                       echo preg_replace("/\r|\n/", "", $tabs_tmp);
                       echo '\',' . "\n";
                       echo '								visibleAmount: ' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_amount', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								hide_onmobile: ' . $hide_tabs_on_mobile . ',' . "\n";
                       if ($hide_tabs_on_mobile === 'true') {
                           echo '								hide_under:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_under_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       if ($hide_tabs_over === 'true') {
                           echo '								hide_over:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_over_hidden', '0', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								hide_onleave:' . $tabs_always_on . ',' . "\n";
                       if ($tabs_always_on === 'true') {
                           echo '								hide_delay:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_tabs', '200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                           echo '								hide_delay_mobile:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_tabs_mobile', '1200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       }
                       echo '								hide_delay:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('hide_tabs', '200', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								direction:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_direction', 'horizontal')) . '",' . "\n";
                       echo '								span:';
                       echo ($this->slider->getParam('span_tabs_wrapper', 'off') == 'on') ? 'true' : 'false';
                       echo ',' . "\n";
                       echo '								position:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_inner_outer', 'inner')) . '",' . "\n";
                       ...
                       echo '								space:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_space', '5', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '								h_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_align_hor', 'left')) . '",' . "\n";
                       echo '								v_align:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_align_vert', 'center')) . '",' . "\n";
                       echo '								h_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_offset_hor', '20', RevSlider::FORCE_NUMERIC)) . ',' . "\n";
                       echo '                                v_offset:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('tabs_offset_vert', '0', RevSlider::FORCE_NUMERIC)) . "\n";
                       echo '							}' . "\n";
                   }
                   echo '						},' . "\n";
               ...
               if ($slider_type == 'carousel') {
                   ...
                   echo '						carousel: {' . "\n";
                   if ($car_borderr > 0) {
                       echo '							border_radius: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_borderr . $car_borderr_unit) . '",' . "\n";
                   }
                   if ($car_padding_top > 0) {
                       echo '							padding_top: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_padding_top) . '",' . "\n";
                   }
                   if ($car_padding_bottom > 0) {
                       echo '							padding_bottom: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_padding_bottom) . '",' . "\n";
                   }
                   if ($carousel_rotation == 'on') {
                       echo '							maxRotation: ' . Mage::helper('nwdrevslider/framework')->esc_attr($car_maxrotation) . ',' . "\n";
                       echo '							vary_rotation: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_varyrotate) . '",' . "\n";
                   }
                   if ($carousel_scale == 'on') {
                       echo '							minScale: ' . Mage::helper('nwdrevslider/framework')->esc_attr($car_scaledown) . ',' . "\n";
                       echo '							vary_scale: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_varyscale) . '",' . "\n";
                   }
                   echo '							horizontal_align: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_hpos) . '",' . "\n";
                   echo '							vertical_align: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_vpos) . '",' . "\n";
                   echo '							fadeout: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_fadeout) . '",' . "\n";
                   if ($car_fadeout == 'on') {
                       echo '							vary_fade: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_varyfade) . '",' . "\n";
                   }
                   echo '							maxVisibleItems: ' . Mage::helper('nwdrevslider/framework')->esc_attr($car_maxitems) . ',' . "\n";
                   echo '							infinity: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_inf) . '",' . "\n";
                   echo '							space: ' . Mage::helper('nwdrevslider/framework')->esc_attr($car_space) . ',' . "\n";
                   echo '                            stretch: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_stretch) . '",' . "\n";
                   echo '                             showLayersAllTime: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_all_layers) . '",' . "\n";
                   echo '                             easing: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_easing) . '",' . "\n";
                   echo '                             speed: "' . Mage::helper('nwdrevslider/framework')->esc_attr($car_speed) . '"' . "\n";
               ...
               if ($label_viewport === 'on') {
                   echo '						viewPort: {' . "\n";
                   echo '							enable:true,' . "\n";
                   echo '							outof:"' . Mage::helper('nwdrevslider/framework')->esc_attr($viewport_start) . '",' . "\n";
                   echo '                            visible_area:"' . Mage::helper('nwdrevslider/framework')->esc_attr($viewport_area) . '%",' . "\n";
                   echo '                            presize:';
               ...
               echo '						lazyType:"' . Mage::helper('nwdrevslider/framework')->esc_attr($lazyLoad) . '",' . "\n";
               ...
               if ($use_scrolleffect == 'on') {
                   ...
                   if ($this->slider->getParam('fade_scrolleffect', 'off') !== 'off')
                       echo '                            fade:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('fade_scrolleffect', 'off')) . '",' . "\n";

                   if ($this->slider->getParam('blur_scrolleffect', 'off') !== 'off')
                       echo '                            blur:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('blur_scrolleffect', 'off')) . '",' . "\n";

                   if ($this->slider->getParam('grayscale_scrolleffect', 'off') !== 'off')
                       echo '                            grayscale:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('grayscale_scrolleffect', 'off')) . '",' . "\n";


                   if ($this->slider->getParam('scrolleffect_maxblur', '10') !== '10')
                       echo '                            maxblur:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_maxblur', '10')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_bg', 'off') !== 'off')
                       echo '                            on_slidebg:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_bg', 'off')) . '",' . "\n";


                   if ($this->slider->getParam('scrolleffect_layers', 'off') !== 'off')
                       echo '                            on_layers:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_layers', 'off')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_parallax_layers', 'off') !== 'off')
                       echo '                            on_parallax_layers:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_parallax_layers', 'off')) . '",' . "\n";


                   if ($this->slider->getParam('scrolleffect_static_layers', 'off') !== 'off')
                       echo '                            on_static_layers:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_static_layers', 'off')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_static_parallax_layers', 'off') !== 'off')
                       echo '                            on_parallax_static_layers:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_static_parallax_layers', 'off')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_direction', 'both') !== 'both')
                       echo '                            direction:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_direction', 'both')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_multiplicator', '1.3') !== '1.3')
                       echo '                            multiplicator:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_multiplicator', '1.3')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_multiplicator_layers', '1.3') !== '1.3')
                       echo '                            multiplicator_layers:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_multiplicator_layers', '1.3')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_tilt', '30') !== '30')
                       echo '                            tilt:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_tilt', '30')) . '",' . "\n";

                   if ($this->slider->getParam('scrolleffect_off_mobile', 'on') !== 'on')
                       echo '                            disable_on_mobile:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('scrolleffect_off_mobile', 'on')) . '",' . "\n";
                   echo '                        },' . "\n";
               }

               if ($use_parallax == 'on') {
                   echo '						parallax: {' . "\n";
                   echo '							type:"' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_type) . '",' . "\n";
                   echo '							origo:"' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_origo) . '",' . "\n";
                   echo '							speed:' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_speed) . ',' . "\n";
                   echo '                          speedbg:' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_bg_speed) . ',' . "\n";
                   echo '                          speedls:' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ls_speed) . ',' . "\n";
                   echo '							levels:[' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_level) . '],' . "\n";

                   if ($parallax_type == '3D') {
                       echo '							ddd_shadow:"' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ddd_shadow) . '",' . "\n";
                       echo '							ddd_bgfreeze:"' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ddd_bgfreeze) . '",' . "\n";
                       echo '							ddd_overflow:"' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ddd_overflow) . '",' . "\n";
                       echo '							ddd_layer_overflow:"' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ddd_layer_overflow) . '",' . "\n";
                       echo '							ddd_z_correction:' . Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ddd_zcorrection) . ',' . "\n";
                       //echo '							ddd_path:"'. Mage::helper('nwdrevslider/framework')->esc_attr($parallax_ddd_path) .'",'."\n";
                   }
               ...
               if ($use_parallax != 'on' || ($use_parallax == 'on' && $parallax_type != '3D'))
                   echo '						shadow:' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("shadow_type", "2")) . ',' . "\n";

               if ($use_spinner == '-1') {
                   echo '						spinner:"off",' . "\n";
               } else {
                   echo '						spinner:"spinner' . Mage::helper('nwdrevslider/framework')->esc_attr($use_spinner) . '",' . "\n";
               }

               if ($slider_type !== 'hero') {
                   echo '						stopLoop:"' . Mage::helper('nwdrevslider/framework')->esc_attr($stopSlider) . '",' . "\n";
                   echo '						stopAfterLoops:' . Mage::helper('nwdrevslider/framework')->esc_attr($stopAfterLoops) . ',' . "\n";
                   echo '						stopAtSlide:' . Mage::helper('nwdrevslider/framework')->esc_attr($stopAtSlide) . ',' . "\n";
                   echo '						shuffle:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("shuffle", "off")) . '",' . "\n";
               }

               echo '						autoHeight:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam("auto_height", 'off')) . '",' . "\n";
               ...
               echo '						hideThumbsOnMobile:"' . Mage::helper('nwdrevslider/framework')->esc_attr($hideThumbsOnMobile) . '",' . "\n";
               echo '						hideSliderAtLimit:' . Mage::helper('nwdrevslider/framework')->esc_attr($hideSliderAtLimit) . ',' . "\n";
               echo '						hideCaptionAtLimit:' . Mage::helper('nwdrevslider/framework')->esc_attr($hideCaptionAtLimit) . ',' . "\n";
               echo '						hideAllCaptionAtLilmit:' . Mage::helper('nwdrevslider/framework')->esc_attr($hideAllCaptionAtLimit) . ',' . "\n";

               if ($slider_type !== 'hero') {
                   $start_with_slide_enable = $this->slider->getParam('start_with_slide_enable', 'off');
                   if ($start_with_slide_enable == 'on') {
                       echo '						startWithSlide:' . Mage::helper('nwdrevslider/framework')->esc_attr($startWithSlide) . ',' . "\n";
                   }
               }
               ...
               if ($this->slider->getParam('ignore_height_changes', 'off') !== 'off') {
                   echo '                            ignoreHeightChanges:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('ignore_height_changes', 'off')) . '",' . "\n";
                   echo '                            ignoreHeightChangesSize:' . intval(Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('ignore_height_changes_px', '0'))) . ',' . "\n";
               }

               echo '							simplifyAll:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('simplify_ie8_ios4', 'off')) . '",' . "\n";

               if ($slider_type !== 'hero')
                   echo '							nextSlideOnWindowFocus:"' . Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('next_slide_on_window_focus', 'off')) . '",' . "\n";
               ...
               echo '    ';
               Mage::helper('nwdrevslider/framework')->do_action('revslider_fe_javascript_output', $this->slider, $this->sliderHtmlID);
               echo '' . "\n";
               echo '			' . "\n";
               ?>
		...
		if($js_to_footer && $this->previewMode == false && $markup_export == false){
            ...
            Mage::helper('nwdrevslider/framework')->add_action('wp_print_footer_scripts', array($this, 'add_inline_js'), 100);
		}
		...
		switch($use_spinner){
			case '1':
			case '2':
				if(!Mage::helper('nwdrevslider/framework')->is_admin()){?><script>
        ...
        if(!$markup_export){ //not needed for html markup export
			if(Mage::helper('nwdrevslider/framework')->has_action('wp_footer', array($this, 'add_inline_double_jquery_error')) === false){
				Mage::helper('nwdrevslider/framework')->add_action('wp_footer', array($this, 'add_inline_double_jquery_error'));
			}
        }
	}
	...
	public function putErrorMessage($message){
		...
		<div style="width:800px;height:300px;margin-bottom:10px;margin:0px auto;">
			<div style="margin: auto; line-height: 40px; font-size: 14px; color: #FFF; padding: 15px; background: #E74C3C; margin: 20px 0px;">
                <?php echo Mage::helper('nwdrevslider')->__("Revolution Slider Error"); ?>: <?php echo $message; ?>
			</div>
		</div>
		<script type="text/javascript">
			(function(jQuery) {
    ...
    public function putSliderBase($sliderID, $gal_ids = array(), $markup_export = false, $settings = array(), $order = array()){
		...
		try{
            ...
            Mage::helper('nwdrevslider/framework')->do_action('revslider_modify_core_settings', $this->slider);
			...
			foreach($gf as $gfk => $gfv){
				...
				if(!empty($gfv['subsets'])){
					foreach($gfv['subsets'] as $ssk => $ssv){
						if(array_search(Mage::helper('nwdrevslider/framework')->esc_attr($gfk.'+'.$ssv), $gfsubsets) !== false){
							$subsets[] = $ssv;
						}
					}
				}
			...
			$backgroundColor = Mage::helper('nwdrevslider/framework')->esc_attr(trim($this->slider->getParam('background_color')));
            if(!empty($backgroundColor)) {
                $containerStyle .= 'background:'.TPColorpicker::get($backgroundColor).';';
            }

			//set padding
			$containerStyle .= 'padding:'.Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('padding','0')).'px;';

			//set margin:
			if($sliderType != 'fullscreen'){

				if($sliderPosition != 'center'){
					$containerStyle .= 'margin-left:'.Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('margin_left', '0', RevSlider::FORCE_NUMERIC)).'px;';
					$containerStyle .= 'margin-right:'.Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('margin_right', '0', RevSlider::FORCE_NUMERIC)).'px;';
				}

				$containerStyle .= 'margin-top:'.Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('margin_top', '0', RevSlider::FORCE_NUMERIC)).'px;';
				$containerStyle .= 'margin-bottom:'.Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('margin_bottom', '0', RevSlider::FORCE_NUMERIC)).'px;';
			}
			...
			if($showBackgroundImage == 'true' || $showBackgroundImage == 'on'){
				$backgroundImage = Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('background_image'));
				$backgroundFit = Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bg_fit', $this->slider->getParam('def-background_fit', 'cover')));
				$backgroundRepeat = Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bg_repeat', $this->slider->getParam('def-bg_repeat', 'no-repeat')));
				$backgroundPosition = Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getParam('bg_position', $this->slider->getParam('def-bg_position', 'center center')));

				if(!empty($backgroundImage))
					$containerStyle .= "background-image:url(".Mage::helper('nwdrevslider/images')->imageUrl($backgroundImage).");background-repeat:".$backgroundRepeat.";background-size:".$backgroundFit.";background-position:".$backgroundPosition.";";
			}
            ...
            $progress_opa =  $this->slider->getParam('progress_opa',false);
			$progressbar_color =  $this->slider->getParam('progressbar_color','#000000');

			if($enable_progressbar !== 'on' || $slider_type == 'hero')
				$timerBar = 'hide';

            $progress_style = ' style="height: '.Mage::helper('nwdrevslider/framework')->esc_attr($progress_height).'px; background: '.TPColorpicker::convert($progressbar_color, $progress_opa).';"';
			...
			if(Mage::helper('nwdrevslider/framework')->is_super_admin() || Mage::helper('nwdrevslider/framework')->is_admin_bar_showing()){
				if(Mage::helper('nwdrevslider/framework')->current_user_can('edit_theme_options')){
					$add_alias = ' data-alias="'.Mage::helper('nwdrevslider/framework')->esc_attr($this->slider->getAlias()).'"';
				}
			}
