...
class RevSliderTemplate {
	...
	const UPDATE_IMAGES_TIME_LIMIT    = 25; // seconds
	const DEFAULT_IMAGE               = 'admin/assets/images/default_template_image.png';
	...
	public function _download_template($uid){
		...
		if(Mage::helper('nwdrevslider/filesystem')->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) { //check here to not flood the server
            $done    = false;
            $count    = 0;
            do{
                $url        = $this->_rslb->get_url('templates');
                $request    = Mage::helper('nwdrevslider/framework')->wp_remote_post($url.'/'.$this->templates_download, array(
				'body' => array(
					'code' => urlencode($code),
				...
				if($response = $request['body']) {
					if($response !== 'invalid'){
						...
						if($ret !== false){
                            ...
                            $return = array('error' => __('Can\'t write the file into the uploads folder "' . $upload_dir['basedir'].$this->templates_path . '", please change permissions and try again!'));
						}
                    }else{
	...
	public function _get_template_list($force = false){
		...
		if(time() - $last_check > 345600 || $force == true){ //4 days
            ...
            do{
                $url        = $this->_rslb->get_url('templates');
                $request    = Mage::helper('nwdrevslider/framework')->wp_remote_post($url.'/'.$this->templates_list, array(
				'body' => array(
					'code' => urlencode($code),
					'shop_version' => urlencode(self::SHOP_VERSION),
					'version' => urlencode(RevSliderGlobals::SLIDER_REVISION),
     ...
     *
     * @param boolean $firstLevelOnly
     */

    public function updateImages($firstLevelOnly = false) {
        $this->_update_images($firstLevelOnly);
    }
	...
	private function _update_images($firstLevelOnly = false){
		...
		if(!empty($templates) && is_array($templates)){
					...
					if( Mage::helper('nwdrevslider/filesystem')->wp_mkdir_p( $upload_dir['basedir'].$this->templates_path ) ) {
						...
						if((!file_exists($file) && !file_exists($file_plugin)) || isset($temp['push_image'])){
                            if($curl !== false){
                                ...
                                if(!Mage::helper('nwdrevslider/framework')->is_wp_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
                                ...
                                $options = array(
                                    'http'=>array(
                                        ...
                                        "Cookie: foo=bar\r\n" .
                                        'User-Agent: Magento/' . Mage::getVersion() . '; ' . Mage::getBaseUrl().' - fgc - '.$count."\r\n"
                                    )
                                );
			...
			if(!empty($templates['slides']) && is_array($templates['slides'])){
				foreach(array_reverse($templates['slides']) as $key => $temp){
					foreach($temp as $k => $tvalues){

                        if ($firstLevelOnly && $k > 0)
				            continue;
	...
	public function getThemePunchTemplateSliders(){
		...
		foreach ((array) Mage::helper('nwdrevslider/framework')->get_option('revslider-addons', array()) as $pluginKey => $plugin) {
	...
	public function _check_file_path($image, $url = false){
		...
		$file_plugin = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . $this->templates_path_plugin . '/' . $image;
		...
		if(file_exists($file)){ //downloaded image first, for update reasons
			...
			if($url){
				$image = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . $this->templates_path_plugin . $image;
			}else{
				$image = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . $this->templates_path_plugin . '/' . $image;
	...
	public function write_import_template_markup($template){
		...
		if($template['img'] == ''){
			$template['img'] = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . self::DEFAULT_IMAGE;
		}
        ...
        <div data-src="<?php echo $template['img']; ?>" class="template_slider_item_import"
			...
			data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"
			<?php
            ...
            <div class="template_preview_add_wrapper">
                ...
                <a class="preview_template_slider" href="<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($template['preview']); ?>" target="_blank"><i class="eg-icon-search"></i></a>
                <?php } ?>
        ...
        <div class="template_thumb_more">
            ...
            if(isset($template['setup_notes']) && !empty($template['setup_notes'])){
                ?>
                <span class="ttm_label"><?php echo __('Setup Notes'); ?></span>
                <?php
            ...
            ?>
            <span class="ttm_label"><?php echo __('Requirements'); ?></span>
            ...
            if($deny == '' && $allow_install == true){
                if(isset($template['package_parent']) && $template['package_parent'] !== ''){
                    ...
                    ?>
                    <div class="install_template_slider<?php echo $deny; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Install Slider'); ?></div>
                    <?php
                }
                if(isset($template['package']) && $template['package'] !== ''){
                    ?>
                    <span class="tp-clearfix" style="margin-bottom:5px"></span>
                    <div class="install_template_slider_package<?php echo $deny; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Install Slider Pack'); ?></div>
                    <?php
                }
            } else {
                ?>
                <div class="dontadd_template_slider_item"><i class="icon-not-registered"></i><?php echo __('Requirements not met'); ?></div>
                <?php
            }
            ?>
            <span class="tp-clearfix" style="margin-bottom:5px"></span>
	...
	public function write_import_template_markup_slide($template){
        ...
        <div class="template_thumb_more">
            ...
            <span class="ttm_space"></span>
            <span class="ttm_label_direct"><span class="ttm_label_half"><?php echo __('Installed Vers.'); ?></span><span class="ttm_label_half"><?php echo __('Available Vers.'); ?></span></span>
            <span class="ttm_label_half ttm_insalled"><span class="ttm_label_half ttm_available"><?php echo $template['version'];?></span>
            <span class="ttm_space"></span>
            <?php
            if($deny == '' && $allow_install == true){
                ?>
                <div class="install_template_slide<?php echo $deny; ?>" data-slidenumber="<?php echo $template['number']; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Install Slider'); ?></div>
                <?php
            } else {
                ?>
                <div class="dontadd_template_slider_item"><i class="icon-not-registered"></i><?php echo __('Requirements not met'); ?></div>
        ...
        if (isset($template["user_template"])) {
            ...
            <div class="template_thumb_more">
                ...
                if ($allow_install !== false) {
                    ...
                    ?>
                    <i class="eg-icon-plus"></i><?php if ($slider_id == false) { echo __('Add Slide'); } else { echo __('Add Slider'); } ?></div>
                    <?php
                    if ($slider_id !== false && isset($template['package']) && $template['package'] !== '' && $template['package_full_installded']) {
                        ?>
                        <div class="add_template_slider_item_package" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-plus"></i><?php echo __('Add Slider Pack'); ?></div>
                        <?php
                    }
                    if($slider_id !== false){
                        ?>
                        <span class="tp-clearfix" style="margin-bottom:5px"></span>
                        <div class="install_template_slider" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo __('Re-Download Slider'); ?></div>
                        <span class="tp-clearfix" style="margin-bottom:5px"></span>
                        <?php
                        if(isset($template['package']) && $template['package'] !== ''){
                            $txt = ($template['package_full_installded']) ? __('Re-Download Slider Pack') : __('Install Slider Pack');
                            ?>
                            <div class="install_template_slider_package" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>" data-title="<?php echo Mage::helper('nwdrevslider/framework')->esc_html($template['title']); ?>"><i class="eg-icon-download"></i><?php echo $txt; ?></div>
                            <span class="tp-clearfix" style="margin-bottom:5px"></span>
                            <?php
                        }
                    }
    ...
    public function create_html_slides($tp_template_slider, $all_slider, $templates){
                    ...
                    if(is_array($m_slider['plugin_require']) && !empty($m_slider['plugin_require'])){
                        foreach($m_slider['plugin_require'] as $k => $pr){
                            if(!isset($plugin_list[$pr['path']])){
                                if(Mage::helper('nwdrevslider/framework')->is_plugin_active(Mage::helper('nwdrevslider/framework')->esc_attr($pr['path']))){
                    ...
                    if(!empty($c_slides)){
                        ...
                        <div style="margin-bottom:30px" class="template_group_wrappers  <?php echo $slidercat.$slidercat_new.' '; if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">
                            ...
                            if(isset($m_slider['preview']) && $m_slider['preview'] !== ''){
                                echo '<a class="template_slide_preview" href="'.Mage::helper('nwdrevslider/framework')->esc_attr($m_slider['preview']).'" target="_blank"><i class="eg-icon-search"></i></a>';
                            }
                            ...
                            foreach($c_slides as $key => $c_slide){
                                ...
                                <div class="temp_slide_single_wrapper">
                                    ...
                                    $c_slide['current_version'] = ($version_installed !== '') ? $version_installed : __('N/A');
                                    ...
                                    <div class="template_meta_line">
                                        <?php if ($isnew) { ?>
                                        <span class="template_new"><?php echo Mage::helper('nwdrevslider')->__("New", "revslider"); ?></span>
                                        <?php } ?>
                                        <?php if ($isupdate) { ?>
                                        <span class="template_new"><?php echo Mage::helper('nwdrevslider')->__("Update", "revslider"); ?></span>
                                        <?php } ?>
                                        <span class="<?php echo $slidercat;?>"><?php echo Mage::helper('nwdrevslider')->__($etikett_a, "revslider");?></span>
                                        <span class="template_installed"><?php echo Mage::helper('nwdrevslider')->__("Installed", "revslider"); ?><i class="eg-icon-check"></i></span>
                                    </div>
                            ...
                            foreach($c_slides as $key => $c_slide){
                                ...
                                <?php
                                    ...
                                    <div class="template_meta_line">
                                        ...
                                        <?php } ?>
                                        <span class="<?php echo $slidercat;?>"><?php echo Mage::helper('nwdrevslider')->__($etikett_a, "revslider");?></span>
                                        <span class="template_notinstalled"><?php echo Mage::helper('nwdrevslider')->__("Not Installed", "revslider"); ?></span>
                                    </div>
        ...
        if(!empty($all_slider)){
            foreach($all_slider as $c_slider){
                ...
                if(!empty($c_slides)){
                    ?>
                    <div class="template_group_wrappers temp_existing">
                        <?php
                        ...
                        foreach($c_slides as $c_slide){
                            ...
                            <?php
                                ...
                                ?>
                                <div class="template_meta_line">
                                    <span class="template_local"><?php echo Mage::helper('nwdrevslider')->__("Local Slide", "revslider");?></span>
                                </div>
                                <div class="template_thumb_title"><?php echo $title; ?></div>
                            </div>
                            <?php
        ...
        <div class="template_group_wrappers temp_custom">
            ...
            if(!empty($templates)){
                ?>
                <div class="template_slider_title"><?php echo Mage::helper('nwdrevslider')->__('User Templates'); ?></div>
                <div class="temp_slides_in_slider_wrapper">
                <?php
                foreach($templates as $template){
                    ...
                    <div class="temp_slide_single_wrapper">
                        ...
                        ?>
                        <div class="template_meta_line">
                            <span class="template_local"><?php echo Mage::helper('nwdrevslider')->__("User Template", "revslider");?></span>
                        </div>
                        <div class="template_thumb_title"><?php echo $title; ?></div>
                    </div>
                    <?php
    ...
    public function create_html_sliders($tp_template_slider){
        ...
        if(!empty($tp_template_slider)){
            foreach($tp_template_slider as $isd => $m_slider){
                ...
                $slidercat = $m_slider['cat'] == 'Revolution Base' ? " template_free " : " template_premium ";
                $etikett_a = $m_slider['cat'] == 'Revolution Base' ? __("Free") : __("Premium");
            ...
            echo Mage::helper('nwdrevslider')->__('No data could be retrieved from the servers. Please make sure that your website can connect to the themepunch servers.');
            echo '</span>';
        }
        ?>
    ...
    public function check_curl_connection(){
        ...
        $curl = Mage::helper('nwdrevslider/curl') ;
