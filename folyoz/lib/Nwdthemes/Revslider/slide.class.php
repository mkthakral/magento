...
class RevSliderSlide extends RevSliderElementsBase{
	...
	public function initByData($record){
        $record = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByData', $record);
		...
		$imageID = false; //RevSliderFunctions::getVal($params, "image_id");
	...
	public function setImageByID($imageID, $size = 'full'){
        $a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setImageByID', array('imageID' => $imageID, 'size' => $size), $this);
	...
	public function setBackgroundType($new_param){
        $new_param = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setBackgroundType', $new_param, $this);
	...
	public function initBySlide(RevSliderSlide $slide){
        $slide = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initBySlide', $slide, $this);
	...
	public function initByStreamData($postData, $slideTemplate, $sliderID, $sourceType, $additions){
        $a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByStreamData', array('postData' => $postData, 'slideTemplate' => $slideTemplate, 'sliderID' => $sliderID, 'sourceType' => $sourceType, 'additions' => $additions), $this);
        ...
        switch($a['sourceType']){
			...
			break;
			default:
                $return = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByStreamData_sourceType', false, $a, $this);
                if($return === false)
                    RevSliderFunctions::throwError(__("Source must be from Stream"));
			break;
		}
	}
	...
	private function initByFacebook($sliderID, $additions){
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFacebook_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fb.png';
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFacebook_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByTwitter($sliderID, $additions){
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByTwitter_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamtwitter' || $this->params["background_type"] == 'streamtwitterboth'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/tw.png';
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByTwitter_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByInstagram($sliderID){
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByInstagram_pre', $this->postData, $sliderID, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streaminstagram' || $this->params["background_type"] == 'streaminstagramboth'){ //if image is choosen, use featured image as background
            ...
            if(empty($this->postData['node']->display_url)){
                $this->imageUrl = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'public/assets/assets/sources/ig.png';
            } else {
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByInstagram_post', $this->postData['node'], $sliderID, $this);
	...
	private function initByFlickr($sliderID){
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFlickr_pre', $this->postData, $sliderID, $this);
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fr.png';
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFlickr_post', $this->postData, $sliderID, $this);
		//replace placeholders in layers:
	...
	private function initByYoutube($sliderID, $additions){
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByYoutube_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamyoutube' || $this->params["background_type"] == 'streamyoutubeboth'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/yt.png';
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByYoutube_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByVimeo($sliderID, $additions){
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByVimeo_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamvimeo' || $this->params["background_type"] == 'streamvimeoboth'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vm.png';
        ...
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByVimeo_post', $this->postData, $sliderID, $additions, $this);
	...
	private function setLayersByStreamData($sliderID, $stream_type, $additions = array()){
        ...
        $a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByStreamData_pre', array('arrLayers' => $this->arrLayers, 'params' => $this->params), $sliderID, $stream_type, $additions, $this);
		...
		foreach($this->arrLayers as $key=>$layer){
			...
			$text = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_mod_stream_meta', $text, $sliderID, $stream_type, $this->postData); //option to add your own filter here to modify meta to your likings
        ...
        $a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByStreamData_post', array('arrLayers' => $this->arrLayers, 'params' => $this->params), $sliderID, $stream_type, $additions, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
        ...
        $text = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_set_stream_data_pre', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
        ...
        foreach ($attrKeys as $key) {
            $text = str_replace(array('%'.$key.'%', '{{'.$key.'}}'), isset($attr[$key]) ? $attr[$key] : '', $text);
        }
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_set_stream_data_post', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
	}
	...
	public function return_stream_data($stream_type, $additions = array()){
        ...
        $attr = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_return_stream_data_pre', $attr, $stream_type, $additions, $img_sizes);
		...
		switch($stream_type){
			...
			case 'instagram':
				...
				$attr['date'] = Mage::helper('nwdrevslider/framework')->date_i18n(Mage::helper('nwdrevslider/framework')->get_option('date_format').' '.Mage::helper('nwdrevslider/framework')->get_option('time_format'), $attr['date']);
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_return_stream_data_post', $attr, $stream_type, $additions, $img_sizes);
	}
	...
	public function get_facebook_timeline_image(){
        ...
        if(!empty($object_id)){
			...
			$image_url = explode('&', isset($array['url']) ? $array['url'] : '');
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_get_facebook_timeline_image', $return, $object_id, $picture, $this);
	}
	...
	public function initByPostData($postData, RevSliderSlide $slideTemplate, $sliderID){
        $this->postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByPostData', $postData, $slideTemplate, $sliderID, $this);

		$postID = $postData["ID"];

		$slideTemplateID = Mage::helper('nwdrevslider/framework')->get_post_meta($postID, 'slide_template', true);
		...
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = Mage::helper('nwdrevslider/framework')->get_permalink($postID);
			$this->params["link"] = str_replace(array("%link%", '{{link}}'), $link, $this->params["link"]);
			...
			foreach($arrMatches as $match){
				...
				$meta = str_replace("%","",$meta);
				$meta = str_replace('_REVSLIDER_', '-', $meta);
				$metaValue = Mage::helper('nwdrevslider/framework')->get_post_meta($postID,$meta,true);
	...
	private function setLayersByPostData($postData,$sliderID){
        $postData = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByPostData_pre', $postData, $sliderID, $this);
		...
		$attr['link'] = Mage::helper('nwdrevslider/framework')->get_permalink($postID);
		...
		$attr['name'] = RevSliderFunctions::getVal($postData, "name");
		$attr['price'] = RevSliderFunctions::getVal($postData, "price");
		$attr['special_price'] = RevSliderFunctions::getVal($postData, "special_price");
		$attr['short_description'] = RevSliderFunctions::getVal($postData, "short_description");
		$attr['description'] = RevSliderFunctions::getVal($postData, "description");
		$attr['view_link'] = RevSliderFunctions::getVal($postData, "view_link");
		$attr['cart_link'] = RevSliderFunctions::getVal($postData, "cart_link");
		$attr['wishlist_link'] = RevSliderFunctions::getVal($postData, "wishlist_link");

		$ptid = Mage::helper('nwdrevslider/framework')->get_post_thumbnail_id($postID);

		$img_sizes = RevSliderBase::get_all_image_sizes();
		$attr['img_urls'] = array();
		foreach($img_sizes as $img_handle => $img_name){
			$featured_image_url = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($ptid, $img_handle);
        ...
        $attr = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByPostData_post', $attr, $postData, $sliderID, $this);

		foreach($this->arrLayers as $key=>$layer){

			$text = RevSliderFunctions::getVal($layer, "text");
			$text = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_mod_meta', $text, $postID, $postData); //option to add your own filter here to modify meta to your likings
	...
	public function set_post_data($text, $attr, $post_id){
		...
		$price = (isset($attr['price'])) ? $attr['price'] : '';
		$special_price = (isset($attr['special_price'])) ? $attr['special_price'] : '';
		$short_description = (isset($attr['short_description'])) ? $attr['short_description'] : '';
		$description = (isset($attr['description'])) ? $attr['description'] : '';
		$view_link = (isset($attr['view_link'])) ? $attr['view_link'] : '';
		$cart_link = (isset($attr['cart_link'])) ? $attr['cart_link'] : '';
		$wishlist_link = (isset($attr['wishlist_link'])) ? $attr['wishlist_link'] : '';
        ...
        $text = Mage::helper('nwdrevslider/framework')->apply_filters( 'rev_slider_insert_meta', $text, $post_id );
		...
		$text = str_replace(array('%name%', '{{name}}'), $name, $text);
		$text = str_replace(array('%price%', '{{price}}'), $price, $text);
		$text = str_replace(array('%special_price%', '{{special_price}}'), $special_price, $text);
		$text = str_replace(array('%short_description%', '{{short_description}}'), $short_description, $text);
		$text = str_replace(array('%description%', '{{description}}'), $description, $text);
		$text = str_replace(array('%view_link%', '{{view_link}}'), $view_link, $text);
		$text = str_replace(array('%cart_link%', '{{cart_link}}'), $cart_link, $text);
		$text = str_replace(array('%wishlist_link%', '{{wishlist_link}}'), $wishlist_link, $text);
		...
		foreach($img_sizes as $img_handle => $img_name){
			...
			$text = str_replace(array('%product_image_url_'.$img_handle.'%', '{{product_image_url_'.$img_handle.'}}'), $url, $text);
			$text = str_replace(array('%product_image_'.$img_handle.'%', '{{product_image_'.$img_handle.'}}'), $tag, $text);
		}
		...
		$arrMatches = array();
		preg_match_all('/%attribute:\w+%/', $text, $arrMatches);

		foreach($arrMatches as $matched){

			foreach($matched as $match) {

				$meta = str_replace("%attribute:", "", $match);
				$meta = str_replace("%","",$meta);
				$meta = str_replace('_REVSLIDER_', '-', $meta);
				$metaValue = Mage::helper('nwdrevslider/framework')->get_post_meta($post_id,$meta,true);
		...
		foreach($arrMatches as $matched){
			foreach($matched as $match) {
				...
				$avatar = Mage::helper('nwdrevslider/framework')->get_avatar_url($authorID,array("size"=>$vals[0]));
    ...
    public function getDataByID($slideid){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getDataByID', $return, $slideid, $this);
    }
	...
	private function setImageByImageID($imageID){
        ...
        $imageID = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setImageByImageID', $imageID, $this);
	...
	public function getArrChildren(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getArrChildren', $this->arrChildren, $this);
	}
	...
	public function getLayers(){
		$this->validateInited();
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getLayers', $this->arrLayers, $this);
	}
	...
	public function saveLayers(){
        ...
        $this->arrLayers = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_saveLayers', $this->arrLayers, $this->static_slide, $this);
	...
	public function saveParams(){
        ...
        $this->params = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_saveParams', $this->params, $this->static_slide, $this);
	...
	public function getLayersForExport($useDummy = false){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getLayersForExport', $arrLayersNew, $this);
	}
	...
	public function getParamsForExport(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getParamsForExport', $arrParams, $this);
	}
	...
	public function getLayersNormalizeText(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getLayersNormalizeText', $arrLayersNew, $this);
	}
	...
	public function getParams(){
		$this->validateInited();
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getParams', $this->params, $this);
	}
	...
	public function getSettings(){
		$this->validateInited();
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getSettings', $this->settings, $this);
	}
	...
	public function createSlide($sliderID,$obj="",$static = false){
        ...
        $arrInsert = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_createSlide', $arrInsert, $sliderID, $static, $this);
    ...
    protected function updateParamsInDB($arrUpdate = array(), $static = false){
        ...
        $this->params = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_updateParamsInDB', array_merge($this->params,$arrUpdate), $this);
	...
	protected function updateLayersInDB($arrLayers = null, $static = false){
        ...
        $arrLayers = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_updateLayersInDB', $arrLayers, $this);
	...
	public function updateSlideFromData($data){
        ...
        $arrUpdate = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_updateSlideFromData_pre', $arrUpdate, $data, $this);

		$this->db->update(RevSliderGlobals::$table_slides,$arrUpdate,array("id"=>$this->id));

        Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_updateSlideFromData_post', $arrUpdate, $data, $this);
	}
	...
	public function updateStaticSlideFromData($data){
        ...
        $arrUpdate = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_updateStaticSlideFromData', $arrUpdate, $data, $this);

		$this->db->update(RevSliderGlobals::$table_static_slides,$arrUpdate,array("id"=>$this->id));

        Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_updateStaticSlideFromData_post', $arrUpdate, $data, $this);
	...
	public function deleteSlide(){
        ...
        Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_deleteSlide', $this->id);
	}
	...
	public function updateTitleByID($data){
        ...
        $arrUpdate = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_updateTitleByID', $arrUpdate, $data, $this);
	...
	public function toggleSlideStatFromData($data){
        ...
        $arrUpdate = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_toggleSlideStatFromData', $arrUpdate, $data, $this);
	...
	public function getUrlImageThumb(){
        ...
        $urlImage = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getUrlImageThumb', $urlImage, $this);
	...
	public function get_image_attributes($slider_type){
		...
		switch($slider_type){
			...
			break;
			case 'posts':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/post.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'woocommerce':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/wc.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'facebook':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fb.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'twitter':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/tw.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'instagram':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/ig.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'flickr':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fr.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'youtube':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/yt.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'vimeo':
				$thumb = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vm.png';
				$bgStyle = 'background-size: cover;';
			break;
		}
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_get_image_attributes', array(
			'url' => Mage::helper('nwdrevslider/images')->imageUrl($data_urlImageForView),
			'class' => $bg_extraClass,
			'style' => $bg_fullstyle
        ), $this);
	...
	public function replaceImageUrls($urlFrom, $urlTo, $static = false){
		...
		foreach($this->arrLayers as $key=>$layer){
            ...
            if(isset($type) && ($type == 'video' || $type == 'audio')){
                ...
                $text_val = RevSliderFunctions::getVal($layer, 'text', false);
                if (strpos($text_val, $urlFrom) !== false) {
                    $text_val = str_replace($urlFrom, $urlTo, $text_val);
                    $this->arrLayers[$key]['text'] = $text_val;
                    $isUpdated = true;
                }
            }
        ...
        Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_replaceImageUrls', $this);
	}
	...
	public function getUsedFonts($full = false){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getUsedFonts', $fonts, $this);
	}
	...
	public function replaceCssClass($css_from, $css_to){
        ...
        Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_replaceCssClass', $css_from, $css_to, $this);
	}
	...
	public function reset_slide_values($values){
		...
		foreach($values as $key => $val){
			if($key !== 'sliderid'){
				$this->params[Mage::helper('nwdrevslider/framework')->esc_attr($key)] = Mage::helper('nwdrevslider/framework')->esc_attr($val);
			}
		}
	...
	public static function translateIntoSizes(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_translateIntoSizes', array(
			'align_hor',
			'align_vert',
			'top',
			'left',
	...
	public function triggerUpdateLayers() {
		$this->updateLayersInDB();
	}

	/**
	 *	Make sure all image layers have correct full url
	 *
	 *	@param	array	Layers
	 *	@return	array
	 */

	private function updateLayersImages($layers = array()) {
		foreach ($layers as $key => $layer) {
            if (isset($layer['bgimage_url']) && ! empty($layer['bgimage_url'])) {
                $layers[$key]['bgimage_url'] = Mage::helper('nwdrevslider/images')->imageUrl($layer['bgimage_url']);
            }
            if (isset($layer['image_url']) && ! empty($layer['image_url'])) {
                $layers[$key]['image_url'] = Mage::helper('nwdrevslider/images')->imageUrl($layer['image_url']);
            }
			if (isset($layer['video_data']) && isset($layer['video_data']->urlAudio) && ! empty($layer['video_data']->urlAudio)) {
				$layers[$key]['video_data']->urlAudio = Mage::helper('nwdrevslider/images')->imageUrl($layer['video_data']->urlAudio);
			}
            if (isset($layer['svg']) && isset($layer['svg']->src) && ! empty($layer['svg']->src)) {
                $layers[$key]['svg']->src = Mage::helper('nwdrevslider/images')->imageUrl($layer['svg']->src);
            }
		}
		return $layers;
	}
	...
	/**
	 *	Make sure all params image layers have correct full url
	 *
	 *	@param	array $params	Params
	 *	@return	array
	 */

	private function updateParamsImages($params = array()) {
		if (isset($params['slide_thumb']) && ! empty($params['slide_thumb'])) {
			$params['slide_thumb'] = Mage::helper('nwdrevslider/images')->imageUrl($params['slide_thumb']);
		}
		return $params;
	}
