...
class RevSliderFacebook {
	...
	*/
	...
	public function get_photo_sets($user_id,$item_count=10,$app_id,$app_secret){
		//photoset params
		$oauth = Mage::helper('nwdrevslider/framework')->wp_remote_fopen("https://graph.facebook.com/oauth/access_token?type=client_cred&client_id=".$app_id."&client_secret=".$app_secret);
        $oauth = json_decode($oauth);
        $url = "https://graph.facebook.com/$user_id/albums?access_token=".$oauth->access_token;
		$photo_sets_list = json_decode(Mage::helper('nwdrevslider/framework')->wp_remote_fopen($url));
	...
	public function get_photo_set_photos($photo_set_id,$item_count=10,$app_id,$app_secret){
		...
		if ($this->transient_sec > 0 && false !== ($data = Mage::helper('nwdrevslider/framework')->get_transient( $transient_name)))
			return ($data);

		$photo_set_photos = json_decode(Mage::helper('nwdrevslider/framework')->wp_remote_fopen($url));

        if(isset($photo_set_photos->data)){
            Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $photo_set_photos->data, $this->transient_sec );
	...
	public function get_photo_set_photos_options($user_url,$current_album,$app_id,$app_secret,$item_count=99){
		...
		if(is_array($photo_sets)){
			foreach($photo_sets as $photo_set){
				$return[] = '<option title="'.$photo_set->name.'" '.Mage::helper('nwdrevslider/framework')->selected( $photo_set->id , $current_album , false ).' value="'.$photo_set->id.'">'.$photo_set->name.'</option>"';
			}
		}
		return $return;
	}
	...
	public function get_photo_feed($user,$app_id,$app_secret,$item_count=10){
		...
		$feed = json_decode(Mage::helper('nwdrevslider/framework')->wp_remote_fopen($url));

		if(isset($feed->data)){
			Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $feed->data, $this->transient_sec );
			return $feed->data;
		}
		else return '';
	}
...
class RevSliderTwitterApi {
        ...
        public function __construct( $args = array() , $transient_sec = 1200 ) {
                ...
                if ( ! $this->bearer_token = Mage::helper('nwdrevslider/framework')->get_option( 'twitter_bearer_token' ) )
        ...
        private function get_bearer_token() {
                ...
                $response = Mage::helper('nwdrevslider/framework')->wp_remote_post( 'https://api.twitter.com/oauth2/token', $args );

                if ( Mage::helper('nwdrevslider/framework')->is_wp_error( $response ) || 200 != $response['response']['code'] )
                        return $this->bail( __( 'Can\'t get the bearer token, check your credentials', 'wp_twitter_api' ), $response );

                $result = json_decode( $response['body'] );

                Mage::helper('nwdrevslider/framework')->update_option( 'twitter_bearer_token', $result->access_token );
        ...
        public function query( $query, $query_args = array(), $stop = false ) {
                ...
                if ($this->query_args['cache'] > 0 && false !== ( $data = Mage::helper('nwdrevslider/framework')->get_transient( $transient_name ) ) )
                ...
                $response = Mage::helper('nwdrevslider/framework')->wp_remote_get( 'https://api.twitter.com/1.1/' . $this->query_args['type'] . '.json?' . $query, $args );
                if ( Mage::helper('nwdrevslider/framework')->is_wp_error( $response ) || 200 != $response['response']['code'] ){
                ...
                Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $response['body'], $this->query_args['cache'] );
        ...
        private function bail( $error_text, $error_object = '' ) {
                ...
                if ( Mage::helper('nwdrevslider/framework')->is_wp_error( $error_object ) ){
  ...
  private $transient_sec;
   ...
   // $url = 'https://api.instagram.com/v1/users/search?q='.$search_user_id.'&access_token='.$this->api_key;
    ...
    $transient_name = 'revslider_' . md5($url);
        ...
        // contact API
        $rsp = json_decode(Mage::helper('nwdrevslider/framework')->wp_remote_fopen($url));

        // set new transient
    if(isset($rsp->user->id))
      Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $rsp->user->id, 604800 );
...
*/
        ...
        if(isset($rsp->tag->media->nodes)){
            ...
            Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $rsp->tag->media->nodes, $this->transient_sec );
...
class RevSliderFlickr {
	...
	*/
	...
	private function call_flickr_api($params){
		...
		if(isset($rsp)){
			Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $rsp, $this->transient_sec );
			return $rsp;
		}
		else return '';
	}
	...
	public function get_photo_sets($user_id,$item_count=10,$current_photoset){
		...
		foreach($photo_sets_list->photosets->photoset as $photo_set){
			...
			$return[] = '<option title="'.$photo_set->description->_content.'" '.Mage::helper('nwdrevslider/framework')->selected( $photo_set->id , $current_photoset , false ).' value="'.$photo_set->id.'">'.$photo_set->title->_content.' ('.$photo_set->photos.' photos)</option>"';
		}
		return $return;
	}
...
class RevSliderYoutube {
	...
	*/
    ...
    if(empty($count)) $count = 50;
		...
		Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $rsp->items, $this->transient_sec );
		return $rsp->items;
	}
	...
	public function get_playlist_options($current_playlist){
		...
		if(!empty($playlists)){
			foreach($playlists as $playlist){
				$return[] = '<option title="'.$playlist->snippet->description.'" '.Mage::helper('nwdrevslider/framework')->selected( $playlist->id , $current_playlist , false ).' value="'.$playlist->id.'">'.$playlist->snippet->title.'</option>"';
			}
		}
		return $return;
	}
