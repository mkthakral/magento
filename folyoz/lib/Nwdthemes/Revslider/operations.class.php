...
class RevSliderOperations extends RevSliderElementsBase{
    ...
    public function __construct() {
        $this->_curl = Mage::helper('nwdrevslider/curl');
	...
	public static function getArrEndAnimations($all = true){
		$arrAnimations = array();
		$arrAnimations['custom'] = array('handle' => __('## Custom Animation ##'));
		$arrAnimations['auto'] = array('handle' => __('Automatic Reverse'));
        $arrAnimations['vSFXs'] = array('handle' => '-----------------------------------');
        $arrAnimations['vSFX'] = array('handle' => __('- SPECIAL EFFECTS -'));
        $arrAnimations['vSFXe'] = array('handle' => '-----------------------------------');
	...
	public function getArrFontFamilys() {
		if ( ! $fonts = Mage::registry('nwdthemes_rs_fonts')) {

			$fonts = array(
				array('type' => 'websafe', 'version' => __('Loaded Google Fonts'), 'label' => 'Dont Show Me'),
				array('type' => 'websafe', 'version' => __('Serif Fonts'), 'label' => 'Georgia, serif'),
				array('type' => 'websafe', 'version' => __('Serif Fonts'), 'label' => '"Palatino Linotype", "Book Antiqua", Palatino, serif'),
				array('type' => 'websafe', 'version' => __('Serif Fonts'), 'label' => '"Times New Roman", Times, serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => 'Arial, Helvetica, sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => '"Arial Black", Gadget, sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => '"Comic Sans MS", cursive, sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => 'Impact, Charcoal, sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => '"Lucida Sans Unicode", "Lucida Grande", sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => 'Tahoma, Geneva, sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => '"Trebuchet MS", Helvetica, sans-serif'),
				array('type' => 'websafe', 'version' => __('Sans-Serif Fonts'), 'label' => 'Verdana, Geneva, sans-serif'),
				array('type' => 'websafe', 'version' => __('Monospace Fonts'), 'label' => '"Courier New", Courier, monospace'),
				array('type' => 'websafe', 'version' => __('Monospace Fonts'), 'label' => '"Lucida Console", Monaco, monospace')
			);

			include(Mage::getBaseDir('lib') . '/Nwdthemes/Revslider/googlefonts.php');

			$version = __('Google Fonts');
			foreach ($googlefonts as $f => $val) {
				$fonts[] = array('type' => 'googlefont', 'version' => $version, 'label' => $f, 'variants' => $val['variants'], 'subsets' => $val['subsets']);
			}

			$fonts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_operations_getArrFontFamilys', $fonts);

			Mage::register('nwdthemes_rs_fonts', $fonts);
		}
		return $fonts;
	}
	...
	public static function getStaticCss(){
		...
		if(!Mage::helper('nwdrevslider/framework')->get_option('revslider-static-css')){
			if(file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/css/static-captions.css')){
				$contentCSS = @file_get_contents(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/css/static-captions.css');
				self::updateStaticCss($contentCSS);
			}
		}
		$contentCSS = preg_replace('/([{};\/])([n \t]+)/', "$1\n", Mage::helper('nwdrevslider/framework')->get_option('revslider-static-css', ''));
	...
	public static function updateStaticCss($content){
		...
		$c = Mage::helper('nwdrevslider/framework')->get_option('revslider-static-css', '');
	...
	public function getDynamicCss(){
		...
		$styles = self::$css;
		$result = RevSliderCssParser::parseDbArrayToCss($styles, "\n");
	...
	public static function getCaptionsCssContentArray(){
		if(file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/css/captions.css'))
			$contentCSS = file_get_contents(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/css/captions.css');
		else if(file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/css/captions-original.css'))
			$contentCSS = file_get_contents(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/css/captions-original.css');
		else if(file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'backup/'.'captions.css'))
			$contentCSS = file_get_contents(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'backup/'.'captions.css');
		else if(file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'backup/'.'captions-original.css'))
	...
	public function previewOutput($sliderID,$output = null, $storeID = 0){
		...
		//put the output html
		$urlPlugin = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/';
		...
		?>
			<html>
				<head>
                    ...
                    Mage::helper('nwdrevslider/framework')->do_action('revslider_preview_slider_head');
                    ?>
				</head>
				<body style="padding:0px;margin:0px;width:100%;height:100%;position:relative;">
					<?php
						$output->putSliderBase($sliderID);
                    Mage::helper('nwdrevslider/framework')->do_action('revslider_preview_slider_footer', $slider, $output);
					?>
				</body>
			</html>
		<?php
...
ob_end_clean();
        ...
        // add missed dot overlay images
        $dotOverlays = array(
            'twoxtwo' => 'gridtile.png',
            'twoxtwowhite' => 'gridtile_white.png',
            'threexthree' => 'gridtile_3x3.png',
            'threexthreewhite' => 'gridtile_3x3_white.png'
        );
        foreach ($dotOverlays as $dotClass => $dotFile) {
            if (strpos($slider_html, 'dottedOverlay:"' . $dotClass . '"') !== false) {
                if( ! $usepcl) {
                    $zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/assets/' . $dotFile, $path_assets_raw.'/'.$dotFile);
                }else{
                    $pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/assets/' . $dotFile, PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/', $path_assets_raw.'/');
                }
            }
        }
		...
		if($export_real){ //only include if real export
			...
			if(!$usepcl){
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.actions.min.js', 'js/extensions/revolution.extension.actions.min.js');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.carousel.min.js', 'js/extensions/revolution.extension.carousel.min.js');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.kenburn.min.js', 'js/extensions/revolution.extension.kenburn.min.js');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.layeranimation.min.js', 'js/extensions/revolution.extension.layeranimation.min.js');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.migration.min.js', 'js/extensions/revolution.extension.migration.min.js');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.navigation.min.js', 'js/extensions/revolution.extension.navigation.min.js');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/js/extensions/revolution.extension.parallax.min.js', 'js/extensions/revolution.extension.parallax.min.js');
				...
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff');

				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/css/font-awesome.css', 'fonts/font-awesome/css/font-awesome.css');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/FontAwesome.otf', 'fonts/font-awesome/fonts/FontAwesome.otf');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.eot', 'fonts/font-awesome/fonts/fontawesome-webfont.eot');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.svg', 'fonts/font-awesome/fonts/fontawesome-webfont.svg');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.ttf', 'fonts/font-awesome/fonts/fontawesome-webfont.ttf');
                $zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff', 'fonts/font-awesome/fonts/fontawesome-webfont.woff');
                $zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff2', 'fonts/font-awesome/fonts/fontawesome-webfont.woff2');

				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.eot', 'fonts/revicons/revicons.eot');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.svg', 'fonts/revicons/revicons.svg');
				$zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.ttf', 'fonts/revicons/revicons.ttf');
                $zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/fonts/revicons/revicons.woff', 'fonts/revicons/revicons.woff');

                $zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/assets/coloredbg.png', 'assets/coloredbg.png');
                $zip->addFile(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'/public/assets/assets/loader.gif', 'assets/loader.gif');
			}else{
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.actions.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.carousel.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.kenburn.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.layeranimation.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.migration.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.navigation.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/js/extensions/revolution.extension.parallax.min.js', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				...
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');

				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/css/font-awesome.css', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/FontAwesome.otf', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.eot', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.svg', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.ttf', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff2', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');

				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.eot', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.svg', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
				$pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.ttf', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.woff', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
                $pclzip->add(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/revicons/revicons.woff2', PCLZIP_OPT_REMOVE_PATH, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/');
	...
	public function modifyCustomSliderParams($data){
        ...
        $data['params']['allow_groups'] = array();
		foreach ($data['params'] as $param => $value) {
            if (strpos($param, 'allow_group_') === 0) {
                if ($value == 'on') {
                    $data['params']['allow_groups'][] = intval(str_replace('allow_group_', '', $param));
                }
  ...
  src: local('".$f_family."'), local('".$f_family."'), url(".$base_url . $fontPath . $font_name . '/' . $font_name . '.woff2'.") format('woff2');
	...
	public function checkPurchaseVerification($data){
        ...
        do {
            ...
            $response = Mage::helper('nwdrevslider/framework')->wp_remote_post($url . '/activate.php', array(
                'body' => array(
		...
		if($version_info == 'valid'){
			...
			Mage::helper('nwdrevslider/framework')->update_option('revslider-code', $data['code']);
            ...
            Mage::helper('nwdrevslider/framework')->update_option('revslider-valid', 'true');
            Mage::helper('nwdrevslider/framework')->update_option('revslider-code', $data['code']);
            Mage::helper('nwdrevslider/framework')->update_option('revslider-temp-active', 'true');
            Mage::helper('nwdrevslider/framework')->update_option('revslider-temp-active-notice', 'false');
            return 'temp';
		}else{
			return false;
	...
	public function doPurchaseDeactivation($data){
        ...
        do {
            $url = $this->_rslb->get_url('updates');
            $response = Mage::helper('nwdrevslider/framework')->wp_remote_post($url . '/deactivate.php', array(
                'body' => array(
                    'code' => urlencode($code),
                    'product' => urlencode(RevSliderGlobals::PLUGIN_SLUG)
                )
		...
		if($version_info == 'valid'){
            ...
            Mage::helper('nwdrevslider/framework')->update_option('revslider-code', '');
			return true;
		}else{
			return false;
		}
	...
	public static function get_slider_speed($sliderID){
			...
			foreach($slides as $key => $slide){
				...
				if(isset($slide['params']['image']) && $slide['params']['image'] != ''){
					...
					if(isset($slide['params']['image_id'])){
						...
						$urlImage = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($slide['params']['image_id'], $slide['params']['image_source_type']);
					}
					if($urlImage === false){
		...
		<span class="tp-monitor-performace-wrap">
			...
			<span class="tp-monitor-slow"><?php echo __("Slow"); ?></span>
		...
		<span  class="tp-monitor-speed-table tp-monitor-single-speed">
			...
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo __("Load Speed DSL:"); ?></span>
				<span class="tp-monitor-total-subsize" id="dsl-speed">%dslspeed-single%</span>
			</span>
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo __("Load Speed T1:"); ?></span>
		...
		<span style="float:left;width:165px">
			<span class="tp-monitor-smalllabel"><?php echo __("Total Slider Size:"); ?></span>
		...
		if(!empty($used_images)){
			?>
			<!-- THE IMAGE PERFORMANCE MESSING -->
			<span class="tp-monitor-performance-title"><?php echo __("Image Performance"); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<span style="float:left;width:40%">
				<span class="tp-monitor-smalllabel"><?php echo __("Images Loaded:"); ?></span>
				<span class="tp-monitor-imageicon"></span>
				<span id="image_sub_size" class="tp-monitor-total-subsize"><?php echo Mage::helper('nwdrevslider/framework')->size_format($img_size,2); ?></span>
				</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-image-details" data-open="</span><?php echo __("Hide Details"); ?>" data-close="</span><?php echo __("Show Details"); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details"); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
			<!-- THE IMAGE LIST -->
				...
				foreach($used_images as $path => $image){
					...
					if(isset($image['size'])) {
						...
						if($image['id'] == 'external'){
							$_li .=   '<span class="tp-monitor-neutral"></span><span class="tp-monitor-size">'.__('extern').'</span>';
						}else{
		...
		?>
			...
			<span style="float:left;width:40%; display:block">
				...
				<?php } else {?>
					<span class="tp-monitor-imageicon"></span>
					<span class="tp-monitor-total-subsize"><?php echo __("Unknown"); ?></span>
				<?php } ?>
			</span>
			<span style="float:left;width:60%; text-align:right;">
		...
		<!-- THE IMAGE PERFORMANCE MESSING -->
		<span class="tp-monitor-performance-title"><?php echo __("CSS Performance"); ?></span>
		<span class="tp-monitor-performace-wrap">
		...
		<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
		<span style="float:left;width:40%">
			<span class="tp-monitor-smalllabel"><?php echo __("CSS Loaded:"); ?></span>
			<span class="tp-monitor-cssicon"></span><span id="css_sub_size" class="tp-monitor-total-subsize">%css_size%</span>
			</span>
		<span style="float:left;width:60%; text-align:right;">
			<span class="tp-monitor-showdetails" data-target="#monitor-CSS-details" data-open="</span><?php echo __("Hide Details"); ?>" data-close="</span><?php echo __("Show Details"); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo __("Show Details"); ?></span></span>
		</span>
		<span class="tp-clearfix" style="height:15px"></span>
		...
		*/
		...
		if($using_parallax == true) $jsfiles['revolution.extension.parallax.js'] = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.parallax.js';
		if($using_navigation == true) $jsfiles['revolution.extension.navigation.min.js'] = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.navigation.min.js';
		if($using_videos == true) $jsfiles['revolution.extension.video.min.js'] = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.video.min.js';
		if($using_actions == true) $jsfiles['revolution.extension.actions.min.js'] = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.actions.min.js';
		if($using_layeranim == true) $jsfiles['revolution.extension.layeranimation.min.js'] = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . '/public/assets/js/extensions/revolution.extension.layeranimation.min.js';
		...
		$content = str_replace("%firstslide_size%", Mage::helper('nwdrevslider/framework')->size_format($firstslide_size,2), $content);
		$content = str_replace("%smartslide_size%", Mage::helper('nwdrevslider/framework')->size_format($smartslide_size,2), $content);
		$content = str_replace("%allslide_size%", Mage::helper('nwdrevslider/framework')->size_format($total_size,2), $content);
...
$presets[] = array (
	...
	public static function create_slider_page($added){
		...
		if(!is_array($added)) return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_create_slider_page', $new_page_id, $added);

		$content = '';
		$page_id = Mage::helper('nwdrevslider/framework')->get_option('rs_import_page_id', 1);
		...
		foreach($added as $sid){
			...
			if($alias !== ''){
				$content .= '{{block type="nwdrevslider/revslider" alias="'.$alias.'"}}' . "\n"; //this way we will reorder as last comes first
			}
		}

		if ($content !== '') {

			$_title = __('Revolution Slider Page') . ' - ' . Mage::helper('nwdrevslider/framework')->sanitize_title( $slider->getTitle() );
			$_id = 'revslider-' . $page_id . '-' . time() . '-' . Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($alias);

			$cmsPageData = array(
				'title' => $_title,
				'root_template' => 'one_column',
				'meta_keywords' => $_title,
				'meta_description' => $_title,
				'identifier' => $_id,
				'content_heading' => $_title,
				'stores' => array(0),
				'content' => $content
			);
			...
			if ($cmsPage) {
				$new_page_id = Mage::helper('cms/page')->getPageUrl($_id);
				$page_id++;
				Mage::helper('nwdrevslider/framework')->update_option('rs_import_page_id', $page_id);
			}
		}

		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_create_slider_page', $new_page_id, $added);
	}
	...
	public function get_installed_cache_plugins(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_installed_cache_plugins', $all);
	...
	public static function import_failed_message($message, $link = false){
		...
		echo '<div style="font-family:arial; width:100%;height:100%;position:absolute;top:0px;left:0px;background-image:url('.Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'admin/assets/images/errorbg.jpg); background-position:center center; background-size:cover;">';
