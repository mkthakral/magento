...
class RevSliderSlider extends RevSliderElementsBase{
	...
	public function initByAlias($alias){
		...
		try{
            ...
            if(!empty($arrAliases) && is_array($arrAliases)){
                ...
                $strAliases = "'".Mage::helper('nwdrevslider/framework')->sanitize_text_field(implode("' or '", $arrAliases))."'";
			}

            $errorMessage = 'Slider with alias <strong>'.Mage::helper('nwdrevslider/framework')->sanitize_text_field(Mage::helper('nwdrevslider/framework')->esc_attr($alias)).'</strong> not found.';
			if(!empty($strAliases))
	...
	private function createUpdateSliderFromOptions($options, $sliderID = null){
        ...
        $title = Mage::helper('nwdrevslider/framework')->sanitize_text_field(RevSliderFunctions::getVal($arrMain, "title"));
        $alias = Mage::helper('nwdrevslider/framework')->sanitize_text_field(RevSliderFunctions::getVal($arrMain, "alias"));
	...
	private function deleteAllSlides(){
        ...
        Mage::helper('nwdrevslider/framework')->do_action('revslider_slider_deleteAllSlides', $this->id);
	}
    ...
    private function duplicateSlider($title = false, $prefix = false){
		...
		if($title === false){
            ...
            if($prefix !== false){
                $newSliderTitle = Mage::helper('nwdrevslider/framework')->sanitize_text_field($title.' '.$params['title']);
                $newSliderAlias = Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($title.' '.$params['title']);
            }else{
                $newSliderTitle = Mage::helper('nwdrevslider/framework')->sanitize_text_field($title);
                $newSliderAlias = Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($title);
            }
			// Check Duplicate Alias
            $sqlTitle = $this->db->fetch(RevSliderGlobals::$table_sliders, $this->db->prepare("alias = %s", array(Mage::helper('nwdrevslider/framework')->sanitize_title($title))));
			if(!empty($sqlTitle)){
				$response = $this->db->fetch(RevSliderGlobals::$table_sliders);
				$numSliders = count($response);
				$newSliderSerial = $numSliders+1;
	...
	public function exportSlider($useDummy = false){
		...
		if(!empty($cfw) && count($cfw) > 0){
			...
			$d = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_exportSlider_usedMedia', $d, $cfw, $sliderParams, $useDummy); //  $arrSlides, $arrStaticSlide,
        ...
        if(!empty($usedSVG)){
            ...
            $content_url_front = Mage::getSingleton('core/design_package')->getSkinUrl('nwdthemes', array('_area' => 'frontend')) . '/';
            $content_url_admin = Mage::getSingleton('core/design_package')->getSkinUrl('nwdthemes', array('_area' => 'adminhtml')) . '/';
            $content_path = Mage::getSingleton('core/design_package')->getSkinBaseDir(array('_area' => 'adminhtml', '_theme' => 'default')) . DS . 'nwdthemes' . DS;

            $ud = Mage::helper('nwdrevslider/framework')->wp_upload_dir();
            $media_url = $ud['baseurl'];
		...
		$enable_slider_pack = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slider_pack_export', false);
    ...
    public function importSliderFromPost($updateAnim = true, $updateStatic = true, $exactfilepath = false, $is_template = false, $single_slide = false, $updateNavigation = true){
        ...
        try{
			...
			$unzipfile = Mage::helper('nwdrevslider/filesystem')->unzip_file( $filepath, $d_path);

            if( Mage::helper('nwdrevslider/framework')->is_wp_error($unzipfile) ){
                define('FS_METHOD', 'direct'); //lets try direct.

                $unzipfile = Mage::helper('nwdrevslider/framework')->unzip_file( $filepath, $d_path);
                if( Mage::helper('nwdrevslider/framework')->is_wp_error($unzipfile) ){
                    $d_path = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'rstemp/';
                    $rem_path = $d_path;
                    $unzipfile = Mage::helper('nwdrevslider/framework')->unzip_file( $filepath, $d_path);
			...
			if($sliderExists){
				...
				if($is_template === false){ //we want to stay at the given alias if we are a template
					...
					$ti = 1;
					while($this->isAliasExistsInDB(Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($talias))){ //set a new alias and title if its existing in database
					...
					if($talias !== $arrInsert['alias']){
                        ...
                        $sliderParams['alias'] = Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($talias);
						$arrInsert['title'] = $talias;
						$arrInsert['alias'] = Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($talias);
                        $json_params = json_encode($sliderParams);
                        $arrInsert['params'] = $json_params;
					}
				}
            ...
            $content_url = Mage::getSingleton('core/design_package')->getSkinUrl('nwdthemes', array('_area' => 'frontend')) . '/';
			...
			foreach($arrSlides as $sl_key => $slide){
                        ...
                        if(isset($layer['type']) && $layer['type'] == 'svg'){
                            if(isset($layer['svg']) && isset($layer['svg']->src)){
                                if (strpos($layer['svg']->src, '/plugins/revslider/') === 0) {
                                    $layer['svg']->src = $content_url . substr($layer['svg']->src, strlen('/plugins/'));
                                } elseif (strpos($layer['svg']->src, '/plugins/') === 0) {
                                    $layer['svg']->src = Mage::helper('nwdrevslider/images')->imageBaseUrl() . substr($layer['svg']->src, 1);
                                } else {
				...
				$d = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_importSliderFromPost_modify_data', $d, 'normal', $d_path);
			...
			if(isset($arrSlider['static_slides']) && !empty($arrSlider['static_slides']) && $import_statics){
				...
				foreach($static_slide as $slide){
					...
					$d = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_importSliderFromPost_modify_data', $d, 'static', $d_path);
            ...
            if($is_template !== false){ //duplicate the slider now, as we just imported the "template"
				if($single_slide !== false){ //add now one Slide to the current Slider
					...
					$mslider = new RevSlider();
					$title = Mage::helper('nwdrevslider/framework')->sanitize_title( RevSliderFunctions::getVal($sliderParams, 'title', 'slider1') );
					$talias = $title;
					$ti = 1;
					while($this->isAliasExistsInDB( Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($talias) )) { //set a new alias and title if its existing in database
						$talias = $title.' '.$ti;
        ...
        Mage::helper('nwdrevslider/framework')->do_action('revslider_slider_imported', $real_slider_id);
	...
	public function copySlideToSlider($data){
		...
		//check if ID exists
		$add_to_slider = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tableSliders WHERE id = %s", $sliderID), Nwdthemes_Revslider_Helper_Query::ARRAY_A);

		if(empty($add_to_slider))
			return __('Slide could not be duplicated');

		//get last slide in slider for the order
		$slide_order = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tableSlides WHERE slider_id = %s ORDER BY slide_order DESC", $sliderID), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		$order = (empty($slide_order)) ? 1 : $slide_order['slide_order'] + 1;

		$slide_to_copy = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tableSlides WHERE id = %s", $slideID), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function getUsedFonts($full = false){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getUsedFonts', $gf);
	}
    ...
    public function getSlidesFromPosts($publishedOnly = false, $gal_ids = array()){
		...
		switch($sourceType){
			...
			break;
			case "current_post":
			    $currentProductId = Mage::helper('nwdrevslider/products')->getCurrentProductId();
	...
	public function getSlidesFromStream($publishedOnly = false){
		...
		$arrPosts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_pre_mod_stream_data', $arrPosts, $sourceType, $this->id);

		while(count($arrPosts) > $max_posts || count($arrPosts) > $max_allowed){
			array_pop($arrPosts);
		}

		$arrPosts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_post_mod_stream_data', $arrPosts, $sourceType, $this->id);
    ...
    public function getSlidesForOutput($publishedOnly = false, $storeID = 0, $lang = 'all',$gal_ids = array()){
        ...
        if($lang == 'all' || $isSlidesFromStream)
	...
	public function getSlidesForExport($useDummy = false){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getStaticSlideForExport', $arrSlidesExport);
	}
	...
	public function getStaticSlideForExport($useDummy = false){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getSlidesForExport', $arrSlidesExport);
	}
    ...
    public function getPostsFromRelated(){
        ...
        $get_relateds = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_related_posts', $query, $post_id);
    ...
    public function getPostsFromPopular($max_posts = false){
        ...
        $args = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_popular_posts', $args, $post_id);
    ...
    public function getPostsFromRecent($max_posts = false){
        ...
        $args = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_latest_posts', $args, $post_id);
    ...
    private function getPostsFromSpecificList($gal_ids = array()){
        ...
        if(!empty($gal_ids) && $gal_ids[0]){
            $strPosts = $gal_ids;
            $strPosts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_set_posts_list_gal', $strPosts, $this->getID());
            $is_gal = true;
        }else{
            if(isset($gal_ids[0])){
                unset($gal_ids[0]);
            ...
            $strPosts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_set_posts_list', $strPosts, $this->getID());
        }
