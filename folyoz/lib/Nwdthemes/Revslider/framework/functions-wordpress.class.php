...
class RevSliderFunctionsWP {
	...
	public static function import_media($file_url, $folder_name) {
		...
		if (@fclose(@fopen($file_url, "r")) && ! @is_dir($file_url)) { //make sure the file actually exists and not a folder
			...
			if($atc_id == false || $atc_id == NULL){
				...
				$attach_id = Mage::helper('nwdrevslider/framework')->wp_insert_attachment($artdata, $artDir.$folder_name.$filename);
			}else{
				$attach_id = $atc_id;
			}

			//generate metadata and thumbnails
			if($attach_data = Mage::helper('nwdrevslider/framework')->wp_generate_attachment_metadata($attach_id, $saveDir)) Mage::helper('nwdrevslider/framework')->wp_update_attachment_metadata($attach_id, $attach_data);
	...
	public static function registerWidget($widgetName){
        Mage::helper('nwdrevslider/framework')->add_action('widgets_init', array('RevSliderFunctionsWP', 'revSliderRegisterWidget'));
    }
	...
	public static function getExcerptById($postID, $limit=55){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getExcerptById', $excerpt, $post, $limit);
	}
	...
	public static function check_for_shortcodes($mid_content){
		if($mid_content !== null){
			if(Mage::helper('nwdrevslider/framework')->has_shortcode($mid_content, 'gallery')){
