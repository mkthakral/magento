...
class RevSliderPluginUpdate {
	...
	public static function update_css_styles(){
		...
		foreach($styles as $key => $attr){
			...
			$settings = json_decode(RevSliderCssParser::fixQuotedFont($attr['settings']), true);
	...
	public static function check_settings_table(){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '".RevSliderGlobals::$table_settings."'") == Mage::getSingleton('core/resource')->getTableName(RevSliderGlobals::$table_settings)) {
			$result = $wpdb->get_row("SELECT `general` FROM ".RevSliderGlobals::$table_settings, Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			if(isset($result['general'])){
				Mage::helper('nwdrevslider/framework')->update_option('revslider-global-settings', $result['general']);
			}
		}
	}
				...
				if($template_id > 0){ //initialize slider to see if it exists. Then copy over the Template Sliders Slides to the Post Based Slider
					foreach($tempSliders as $t_slider){
						if($t_slider->getID() === $template_id){ //copy over the slides
							...
							// replace product image layer with caption layer

							$slides = $t_slider->getSlides();
							foreach ($slides as $slide) {
								$layers = $slide->getLayers();
								foreach ($layers as $key => $layer) {
									if ($layer['type'] == 'image' && strpos($layer['image_url'], 'product_image_placeholder.png') !== false)
									{
										$layer['type'] = 'text';
										$layer['text'] = '{{product_image_base}}';
										$layers[$key] = $layer;
									}
								}
								$slide->setLayersRaw($layers);
								$slide->triggerUpdateLayers();
							}
                                ...
                                if(isset($layer['type']) && $layer['type'] == 'svg'){
                                    if(isset($layer['svg']) && isset($layer['svg']->src)){
                                        ...
                                        if(strpos($layers[$lk]['svg']->src, $path) !== false){
                                            ...
                                            $layers[$lk]['svg']->src = str_replace($path, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'public/assets/assets/svg/', $layers[$lk]['svg']->src);
                                        }
                                    }
                                }
                            }
