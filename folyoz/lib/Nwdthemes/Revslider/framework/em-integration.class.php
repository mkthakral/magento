...
class RevSliderEventsManager {
	...
	public static function getArrFilterTypes(){
			...
			$arrEventsSort = array("none" => __("All Events"),
								   "today" => __("Today"),
	...
	public static function getWPQuery($filterType, $sortBy){
		...
		$time = Mage::helper('nwdrevslider/framework')->current_time('timestamp');
	...
	public static function getEventPostData($postID){
		...
		$date_format = Mage::helper('nwdrevslider/framework')->get_option('date_format');
		$time_format = Mage::helper('nwdrevslider/framework')->get_option('time_format');
	...
	public static function getArrSortBy(){
		$arrSortBy = array();
		$arrSortBy["event_start_date"] = __("Event Start Date");
		$arrSortBy["event_end_date"] = __("Event End Date");
		return($arrSortBy);			
	}
