...
class Rev_addon_Admin {
	...
	public function add_plugin_admin_menu() {
		$this->plugin_screen_hook_suffix = Mage::helper('nwdrevslider/framework')->add_submenu_page(
			'revslider',
	...
	public function activate_plugin() {
		...
		if( Mage::helper('nwdrevslider/framework')->wp_verify_nonce( Nwdthemes_Revslider_Helper_Data::$_REQUEST['nonce'], 'ajax_rev_slider_addon_nonce' ) ) {
			if(isset(Nwdthemes_Revslider_Helper_Data::$_REQUEST['plugin'])){
				$result = Mage::helper('nwdrevslider/framework')->activate_plugin( Nwdthemes_Revslider_Helper_Data::$_REQUEST['plugin'] );
	...
	public function deactivate_plugin() {
		...
		if( Mage::helper('nwdrevslider/framework')->wp_verify_nonce( Nwdthemes_Revslider_Helper_Data::$_REQUEST['nonce'], 'ajax_rev_slider_addon_nonce' ) ) {
			if(isset(Nwdthemes_Revslider_Helper_Data::$_REQUEST['plugin'])){
				$result = Mage::helper('nwdrevslider/framework')->deactivate_plugins( Nwdthemes_Revslider_Helper_Data::$_REQUEST['plugin'] );
				if ( Mage::helper('nwdrevslider/framework')->is_wp_error( $result ) ) {
					// Process Error
					die('0');
				}
		...
		if( Mage::helper('nwdrevslider/framework')->wp_verify_nonce( Nwdthemes_Revslider_Helper_Data::$_REQUEST['nonce'], 'ajax_rev_slider_addon_nonce' ) ) {
			if(isset(Nwdthemes_Revslider_Helper_Data::$_REQUEST['plugin'])){
                ...
                do{
                    ...
                    $url .= '/magento/addons/'.$plugin_slug.'/'.$plugin_slug.'.zip';
				...
				$get = Mage::helper('nwdrevslider/framework')->wp_remote_post($url, array(
                    ...
                    $response_code = Mage::helper('nwdrevslider/framework')->wp_remote_retrieve_response_code( $get );
	...
	/**
     * Update Add-On from server
     */

	public function updateAddon() {
        $pluginSlug = Mage::app()->getRequest()->getParam('plugin');
        if ($pluginSlug && Mage::helper('nwdrevslider/plugin')->updatePlugin($this->_rslb->get_url('updates'), $pluginSlug)) {
            $message = __('Add-On updated.');
        } else {
            $message = __('Failed to update Add-On.');
        }
        Mage::helper('nwdrevslider/framework')->dmp($message . ' ' . __('Redirecting...'));
        echo "<script>location.href='" . Mage::helper("adminhtml")->getUrl('adminhtml/nwdrevslider/addon') . "'</script>";
        exit();
    }
