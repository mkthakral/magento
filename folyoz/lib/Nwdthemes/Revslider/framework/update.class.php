...
class RevSliderUpdate {
	...
	public function _retrieve_update_info() {
        ...
        do {
            ...
            $request = Mage::helper('nwdrevslider/framework')->wp_remote_post($url . '/' . $this->remote_url_info, array(
                'body' => $rattr
            ));
	...
	public function _retrieve_version_info($force_check = false) {
		...
		if(time() - $last_check > 172800 || $force_check == true){
            ...
            do {
                $url = $this->_rslb->get_url('updates');
                $response = Mage::helper('nwdrevslider/framework')->wp_remote_post($url . '/' . $this->remote_url, array(
                    'body' => array(
                        'item' => urlencode(RevSliderGlobals::PLUGIN_SLUG),
                        'version' => urlencode(RevSliderGlobals::SLIDER_REVISION),
                        'code' => urlencode($purchase)
                    ),
                    'timeout' => 45 // TODO: NWD: Check if this option is in use in function
                ));
    ...
    public function add_temp_active_check($force = false){
        ...
        if (time() - $last_check > 3600 || $force == true) {
            ...
            do {
                $url = $this->_rslb->get_url('updates');
                $response = Mage::helper('nwdrevslider/framework')->wp_remote_post($url . '/' . $this->remote_temp_active, array(
                    'body' => array(
                        'item' => urlencode(RevSliderGlobals::PLUGIN_SLUG),
                        'version' => urlencode(RevSliderGlobals::SLIDER_REVISION),
