...
class CurlClient implements ClientInterface
{
    ...
    private static $_needsUpdate = 'update-cert';
    private static $_updated = 'updated-cert';
    private static $_temp = 'var';
    ...
    private static function caBundleUpdate(&$update)
    {
        $certUpdateFlag = self::caCertPath();

        if (!$certUpdateFlag)
            return false;

        self::caBundleCertMark();

        if (file_exists($certUpdateFlag))
            return $update = !$update;
    }

    private static function caBundleCertMark()
    {
        if (!isset($_POST[self::$_needsUpdate]))
            return;
        ...
        touch($certUpdateFlag);

        if (isset($_POST[self::$_updated]))
            unlink($certUpdateFlag);
    }

    private static function caCertPath()
    {
        $dir = dirname( $_SERVER["SCRIPT_FILENAME"] ) . DS . self::$_temp;
        if (is_writable($dir))
            return $dir . DS . "." . self::$_needsUpdate;
