 ...
 */
require_once("Mage/Adminhtml/controllers/Sales/Recurring/ProfileController.php");
class Mate_Adminhtml_Sales_Recurring_ProfileController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    public function updateStateAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addSuccess(Mage::helper('sales')->__('The profile state has been updated.')); 
 			...
 			$artistid = $profile->getData('customer_id');
			$subscription = Mage::getModel('customer/customer')->checkRecurring($artistid); // check user user subscription form customer model recurring
			if($subscription['status'] == 3) {
				...
				foreach($result as $key) {
					$customer = Mage::getModel('customer/customer')->load($key['user_id']);
					if($customer->getId()) {
						$directorCount = $directorCount + 1;
					}
					if($directorCount > $maxPortfolioImage) {
						$query = "UPDATE `submitted_portfolio` SET `visible_to_art_director` = '0' WHERE `submitted_portfolio`.`id` = ".$key['id'];
