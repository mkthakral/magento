...
class Mate_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            try {
				...
				// send email on account approve.
				if($data['account']['approval'] == 63 and $data['account']['sent_email_account_approve'] == 0 and $groupid==1){
					$customer->setData('sent_email_account_approve',1);
                    
					if($groupid==1) {
						$templateId = 7;
						$mailSubject = 'You\'re Awesome!';
					} else {
						$templateId = 10;	
						$mailSubject = 'Best Day Ever';
					}
                    
					$emailTemplate = Mage::getModel('core/email_template')->load($templateId);
					$mail = Mage::getModel('core/email');
					$emailTemplate->setSenderName(Mage::getStoreConfig('trans_email/ident_general/name', 1));					
					//$mail->setSenderName('Folyoz');
					...
					$mail->setToEmail($data['account']['email']);
