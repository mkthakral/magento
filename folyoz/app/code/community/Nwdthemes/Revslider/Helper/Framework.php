...
class Nwdthemes_Revslider_Helper_Framework extends Mage_Core_Helper_Abstract {
	...
	public function __construct() {
		...
		self::$RS_PLUGIN_PATH = Mage::getSingleton('core/design_package')->getSkinBaseDir(array('_theme' => 'default')) . '/nwdthemes/revslider/';
		list(self::$RS_PLUGIN_URL) = explode('?', Mage::getDesign()->getSkinUrl('nwdthemes/revslider/'));

        $this->_pluginHelper = Mage::helper('nwdrevslider/plugin');
	}
	...
	{
		return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . self::WP_CONTENT_DIR;
	}
	...
	public function current_time() {
        return Mage::app()->getLocale()->storeTimeStamp( Mage::app()->getStore()->getId() );
	}
	...
	public function apply_filters($handle, $value) {
        ...
        if ($this->getFromRegister('actions', $handle)) {
            $value = $this->do_action($handle, $value);
        }
	...
	public function get_post_meta($id, $handle, $flag = true) {
	    if (strpos($handle, 'wp_') === false) {
            $product = Mage::helper('nwdrevslider/products')->getProduct($id, false);
            $meta = isset($product[$handle]) ? $product[$handle] : false;
        } else {
            $meta = false;
        }
		return $meta;
    ...
    public function wp_remote_post($url, $args = array()) {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'Magento/' . Mage::getVersion() . '; ' . Mage::getBaseUrl());
    ...
    public function plugins_url($file, $plugin) {
        return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . Nwdthemes_Revslider_Helper_Plugin::WP_PLUGIN_DIR . basename($plugin, '.php') . '/' . $file;
    }
    ...
    public function plugin_dir_path($plugin) {
        return Nwdthemes_Revslider_Helper_Plugin::getPluginDir() . basename($plugin, '.php') . '/';
    }
