...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    protected function _folyozValidateFormKey(){
        
        if (!($formKey = $this->getRequest()->getParam('form_key', null))
            || $formKey != 'yjyCexAtQjsuLGTV'.Mage::getSingleton('core/session')->getFormKey().'iqjQNFkjNiWxxwaBUo') {
            return false;
        }
    ...
    public function createPostAction()
    {
        ...
        Mage::log('Creating Account from createPostAction() Email: '.$this->getRequest()->getParam('email', null),null,'login.log',true);
        
        if (!$this->_folyozValidateFormKey()) {
            $session = $this->_getSession();
            $session->addError("Are you a bot?");
